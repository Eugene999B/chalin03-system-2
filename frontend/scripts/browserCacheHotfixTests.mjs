...
assert.match(serviceWorker, /new URL\(sourceUrl\)\.origin === self\.location\.origin/);
...
assert.match(mainEntry, /browser-cache-integrity-v52-/);
assert.match(mainEntry, /CACHE_RECOVERY_KEY = "__chalin03_frontend_recovery_v52__"/);
...
assert.match(viteConfig, /"import\.meta\.env\.VITE_CHALIN03_BUILD_ID"/);

assert.match(indexHtml, /<script src="\/browser-cache-recovery\.js"><\/script>/);
...
assert.match(headers, /\/sw\.js\s+Cache-Control: no-store, max-age=0, must-revalidate/);
assert.match(headers, /\/browser-cache-recovery\.js\s+Cache-Control: no-store, max-age=0, must-revalidate/);
...
console.log("✅ Browser cache recovery contracts passed: one page-owned recovery flow loads the root shell, restores the staff's deep route without a second network request, and self-heals unexpected navigation 404 responses.");