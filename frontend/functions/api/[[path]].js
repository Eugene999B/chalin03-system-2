...
function upstreamHeadersFor(request) {
  ...
  // These values belong to the browser-to-Pages hop. Cloudflare will provide
  // fresh edge/origin metadata for the Pages-to-API subrequest.
  for (const header of [
    "cf-connecting-ip",
    "cf-ipcountry",
    "cf-ray",
...
function responseHeadersFor(upstreamResponse) {
  ...
  // The browser is calling chalin03.com/api on the same origin, so upstream
  // cross-origin policy headers are unnecessary and can otherwise be confusing
  // when diagnosing the two separate network hops.
