...
].map(([code, title, short_title, category, formats]) => ({
  code,
  title,
  short_title,
  category,
...
test("staff issues customer agreement and exact-payment thermal receipt from immutable history", async ({ page }) => {
  const issued = [];
  const issuePayloads = [];
  ...
  await page.route("http://api.test/api/**", async (route) => {
    ...
    const downloadMatch = path.match(
      /^\/equipment-catalogue\/sales\/professional\/completion-documents\/(\d+)\/download$/
    );
    if (downloadMatch && method === "GET") {
      ...
      return route.fulfill({
        ...
        contentType,
        headers: {
          "Content-Disposition": `attachment; filename="finance-document.${extension}"`,
        },
        body,
      });
    }
    ...
    if (
      path === "/equipment-catalogue/sales/protected-images/applications/501" &&
      method === "GET"
    ) {
  ...
  await page.goto(
    "/equipment-installment-finance/applications?stage=generated-documents&agreement=601"
  );

  await expect(page.getByRole("heading", { name: "Finance Document Centre" })).toBeVisible();
  await expect(page.getByText("Agreement & Approval Pack")).toBeVisible();
  await expect(page.getByText("Payments & Customer Account")).toBeVisible();
  await expect(page.getByText("Machine, Guarantor & Handover")).toBeVisible();
  await expect(page.getByText("Changes, Settlement & Ownership")).toBeVisible();
  ...
  const customerCopy = page
    .locator(".finance-docs__document-card")
    .filter({ hasText: "Customer Copy" });
  ...
  await expect(page.getByText("EFAC-DOCUMENT-001")).toBeVisible();
  ...
  const paymentReceipt = page
    .locator(".finance-docs__document-card")
    .filter({ hasText: "Payment Receipt" });
  ...
  await expect(page.getByText("EFR-DOCUMENT-001")).toBeVisible();
