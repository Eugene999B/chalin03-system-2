...
test("staff selects an account and exact payment before issuing immutable documents", async ({ page }) => {
  const issued = [];
  const issuePayloads = [];
  ...
  await page.route("http://api.test/api/**", async (route) => {
    ...
    if (path === "/equipment-catalogue/sales/professional/completion-documents/issue" && method === "POST") {
      ...
      const document = {
        id,
        agreement_id: 601,
        document_number: payload.document_type === "payment_receipt" ? "EFR-DOCUMENT-001" : "EFAC-DOCUMENT-001",
