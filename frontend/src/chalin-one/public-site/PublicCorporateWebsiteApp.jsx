      ...
      <div className="c1-site">
        ...
        <header className="c1-header">
          ...
          <nav className="c1-nav" data-open={menuOpen ? "true" : "false"} aria-label="Main website navigation">
            {TOP_NAV.map((item) => <Link key={item.to} to={item.to} className={location.pathname === item.to || location.pathname.startsWith(`${item.to}/`) ? "is-active" : ""}>{item.label}</Link>)}
          </nav>
...
function PageHero({ eyebrow, title, text, media, deep = false }) {
  return (
    <header className={`c1-page-hero${media?.url ? " has-media" : ""}${deep ? " c1-page-hero-deep" : ""}`}>
...
function HomePage() {
  ...
  return (
    <main className="c1-home">
      ...
      {page?.sections?.length ? <section className="c1-governed-sections">{page.sections.filter((section) => section.type !== "hero").map((section, index) => <article className="c1-governed-block" key={section.key || index}><div><span>GOVERNED CONTENT / {String(index + 1).padStart(2, "0")}</span>{section.heading ? <h2>{section.heading}</h2> : null}{section.subheading ? <p className="c1-governed-subtitle">{section.subheading}</p> : null}<StructuredContent value={section.content} /></div><CorporateMedia media={section.primary_media} /></article>)}</section> : null}
...
function AboutPage() {
  ...
  return (
    <main className="c1-page c1-deep-page">
      ...
      <section className="c1-company-architecture"><header><span>02 / COMPANY ARCHITECTURE</span><h2>One group. Three operating worlds.</h2></header><div className="c1-company-grid">{businesses.map((business, index) => {
        const fallback = BUSINESS_FALLBACKS[index] || BUSINESS_FALLBACKS[0];
        const slug = business.slug || fallback.slug;
        return <Link to={`/businesses/${slug}`} key={business.key || slug}><span>{fallback.number}</span><small>{business.label || fallback.label}</small><h3>{business.name || fallback.name}</h3><p>{business.short_description || business.description || fallback.description}</p><b>Enter business world ↗</b></Link>;
      })}</div></section>
...
function BusinessesPage() {
  ...
  return (
    <main className="c1-page c1-deep-page">
      <PageHero deep eyebrow="Business worlds" title="Different missions deserve different worlds." text="Enter each specialist CHALIN business through its own visual, service and enquiry context." />
      <section className="c1-business-orbit">{items.map((business, index) => {
        const fallback = BUSINESS_FALLBACKS[index] || businessFallbackForSlug(business.slug) || BUSINESS_FALLBACKS[0];
        const slug = business.slug || fallback.slug;
        return <Link to={`/businesses/${slug}`} key={business.key || slug}><span>{fallback.number}</span><h2>{business.name || fallback.name}</h2><p>{business.short_description || business.description || fallback.description}</p><b>↗</b></Link>;
      })}</section>
      <section className="c1-values-stage"><header><span>HOW TO ENTER</span><h2>Start with the business—or start with your need.</h2></header><IntentMatrix /></section>
      <DeepCta title="Not sure which business fits?" primary="Use the Navigator" primaryTo="/contact" secondary="General enquiry" secondaryTo="/contact" />
    </main>
  );
}
...
function BusinessDetailPage() {
  ...
  const request = useRequest(async (signal) => {
    ...
    return {
      item: divisionResult.status === "fulfilled" ? divisionResult.value : null,
      projects: projectsResult.status === "fulfilled" ? listFromResult(projectsResult.value) : [],
      equipment: equipmentResult.status === "fulfilled" ? listFromResult(equipmentResult.value) : [],
    };
  }, [slug]);
  if (request.loading) return <main className="c1-page"><LoadingState /></main>;
  ...
  return (
    <main className={`c1-page c1-deep-page c1-world-${world.symbol.toLowerCase()}`}>
      ...
      {currentSection === "overview" ? <>
        <section className="c1-world-intro"><div>{world.symbol}</div><article><span>THE BUSINESS</span><h2>{world.label}</h2><StructuredContent value={item.body || item.details || item.description || world.description} /><p><strong>{world.pulse}</strong></p></article></section>
        <ServiceLanes world={world} />
        <BusinessProof projects={projects} title="Published work from this business." />
        {world === BUSINESS_FALLBACKS[2] ? <BusinessEquipment equipment={equipment} /> : null}
      </> : null}
...
function ServiceLanes({ world }) {
  return <section className="c1-service-lanes"><header><span>SERVICE PATHWAYS</span><h2>Three clear ways into this business.</h2></header><div className="c1-lane-grid">{world.lanes.map(([title, text], index) => <article key={title}><span>0{index + 1}</span><strong>{title}</strong><p>{text}</p></article>)}</div></section>;
}

function BusinessProof({ projects, title }) {
  return <section className="c1-proof-rail"><header><span>PROJECT SIGNAL</span><h2>{title}</h2></header>{projects.length > 0 ? <div className="c1-proof-grid">{projects.slice(0, 5).map((project, index) => <Link className="c1-proof-card" to={`/projects/${project.slug}`} key={project.key || project.slug}>{project.media?.url ? <CorporateMedia media={project.media} /> : <div className="c1-card-placeholder">0{index + 1}</div>}<div><small>{project.division?.name || humanize(project.status || "Project")}</small><h3>{project.title}</h3><p>{project.summary || project.location || project.location_text || ""}</p></div></Link>)}</div> : <EmptyState message="Projects tied to this business will appear here after approval and publication." />}</section>;
}

function BusinessEquipment({ equipment }) {
  return <section className="c1-equipment-lab"><header><span>EQUIPMENT SIGNAL</span><h2>Published machines inside this business world.</h2></header>{equipment.length > 0 ? <div className="c1-equipment-grid">{equipment.slice(0, 5).map((item, index) => <Link className="c1-equipment-card-deep" to={`/equipment/${item.slug}`} key={item.key || item.slug}>{item.media?.url ? <CorporateMedia media={item.media} /> : <div className="c1-card-placeholder">EQ{index + 1}</div>}<div><small>{item.manufacturer || item.category?.name || "Equipment"}</small><h2>{item.name}</h2><p>{item.short_description || [item.manufacturer, item.model].filter(Boolean).join(" ")}</p></div></Link>)}</div> : <EmptyState message="Approved equipment linked to this business will appear here." />}</section>;
}
...
function CollectionPage({ resource, titleOverride }) {
  ...
  return (
    <main className="c1-page c1-deep-page">
      <PageHero deep eyebrow={config.eyebrow} title={title} text={config.description} />
      <section className="c1-collection">{items.length === 0 ? <EmptyState message={`The ${config.eyebrow.toLowerCase()} experience is ready for approved public content.`} /> : <div className="c1-collection-grid">{items.map((item, index) => {
...
function EquipmentExperience({ items }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return items;
    return items.filter((item) => `${item.name || ""} ${item.manufacturer || ""} ${item.model || ""} ${item.category?.name || ""}`.toLowerCase().includes(q));
  }, [items, query]);
...
function DetailPage({ resource }) {
  ...
  return (
    <main className="c1-page c1-deep-page">
      <PageHero deep eyebrow={config.eyebrow} title={config.label(item)} text={config.summary(item)} media={item.media || item.document} />
      ...
      <DeepCta title="Continue from this story." primary="Start an enquiry" primaryTo="/contact" secondary="Back to the archive" secondaryTo={`/${resource === "vacancies" ? "careers" : resource}`} />
    </main>
  );
}
...
function ContactPage({ formSlug = "contact" }) {
  ...
  async function submit(event) {
    ...
    setSubmitError("");
    try {
      const result = await submitPublicForm(form.slug, { ...contact, consent_text_version: form.settings?.consent_text_version || "privacy-v1", source_page_slug: "contact", source_url: window.location.href, responses });
      setSuccess(result);
    } catch (error) {
