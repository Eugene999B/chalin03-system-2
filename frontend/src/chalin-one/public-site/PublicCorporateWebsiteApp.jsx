...
const VISITOR_INTENTS = Object.freeze([
  {
    ...
    index: "01",
    title: "Find a part",
    text: "Route a spare-parts request to the right team with a clear service context.",
    action: "/contact?intent=parts",
    tag: "Spare Parts",
  },
  {
    key: "hire",
    ...
    text: "Start an equipment-hire conversation around the machine, work and timing you need.",
    action: "/contact?intent=hire",
    tag: "Equipment",
  },
  {
    key: "buy",
    index: "03",
    title: "Buy equipment",
    text: "Explore published equipment first, then move directly into a commercial enquiry.",
    action: "/equipment",
    tag: "Sales",
  },
  {
    key: "mining",
    index: "04",
    title: "Mining enquiry",
    text: "Reach the mining team without mixing your request into another CHALIN business.",
    action: "/contact?intent=mining",
    tag: "Mining",
  },
  {
    key: "career",
    index: "05",
    title: "Work with us",
    text: "Discover published vacancies and company opportunities through a dedicated path.",
    action: "/careers",
    tag: "Careers",
  },
  {
    key: "supplier",
    index: "06",
    title: "Supplier or tender",
    text: "Move into published procurement opportunities or make a structured company enquiry.",
    action: "/tenders",
    tag: "Partners",
  },
]);
...
function EmptyState({ message = "Approved content will appear here when it is published." }) {
  return (
    <div className="c1-state c1-state-empty">
      <span>CHALIN ONE / PUBLISHING SIGNAL</span>
...
function ScrollSignal() {
  ...
  return (
    <div className="c1-scroll-signal" aria-hidden="true">
  ...
  return (
    <div className="c1-navigator" role="dialog" aria-modal="true" aria-label="CHALIN ONE navigator">
      ...
      <section className="c1-navigator-panel">
        <div className="c1-navigator-head">
          <div>
            <span>CHALIN ONE / NAVIGATOR</span>
            <h2>What are you here to do?</h2>
          </div>
          <button type="button" onClick={onClose} aria-label="Close navigator">Close ×</button>
        </div>
        <div className="c1-navigator-grid">
          {VISITOR_INTENTS.map((intent) => (
            <button type="button" key={intent.key} onClick={() => go(intent.action)}>
              <span>{intent.index}</span>
              <small>{intent.tag}</small>
        ...
        <div className="c1-navigator-foot">
          <button type="button" onClick={() => go("/businesses")}>Explore the whole group</button>
          <button type="button" onClick={() => go("/contact")}>General company enquiry</button>
  ...
  return (
    ...
    >
      <div className="c1-site">
        ...
        <aside className="c1-compass" aria-hidden="true">
          <b>C1</b>
          <span>03 BUSINESSES</span>
          <i />
          <small>GH / WEST AFRICA</small>
        </aside>
        ...
        {(bootstrap.announcements || []).slice(0, 1).map((announcement) => (
          <div className="c1-announcement" key={announcement.key || announcement.title}>
            ...
            {announcement.link_url ? (
              <a href={announcement.link_url}>{announcement.link_label || "Open update"} ↗</a>
            ) : null}
          </div>
        ))}
        ...
        <header className="c1-header">
          ...
          <div className="c1-access-links">
            <button className="c1-navigator-trigger" type="button" onClick={() => setNavigatorOpen(true)}>
              <span>Explore</span><kbd>⌘K</kbd>
            </button>
            <Link className="c1-access-staff" to="/login">
              <span>Staff portal</span><b>↗</b>
            </Link>
          </div>
        ...
        {request.loading ? (
          <main className="c1-page c1-page-loading">
            <LoadingState label="Opening the CHALIN ONE universe…" />
          </main>
        ) : request.error ? (
          ...
          <div className="c1-route-stage" key={location.pathname}>
            <Outlet />
          </div>
        )}

        <button className="c1-signal-dock" type="button" onClick={() => setNavigatorOpen(true)}>
          <span>START HERE</span>
          <b>+</b>
        </button>

        <footer className="c1-footer">
          <div className="c1-footer-statement">
            <span>CHALIN ONE</span>
            <h2>Three businesses.<br />One signal to the world.</h2>
...
function IntentMatrix() {
  return (
    <section className="c1-intent-section">
      <div className="c1-intent-heading">
        <span>00 / START WITH PURPOSE</span>
        <h2>Don’t browse.<br />Enter through your goal.</h2>
        <p>CHALIN ONE changes the usual corporate website journey. Choose what you need and move directly into the right business context.</p>
      </div>
      <div className="c1-intent-grid">
        {VISITOR_INTENTS.map((intent) => (
          <Link to={intent.action} key={intent.key}>
            <span>{intent.index}</span>
            <small>{intent.tag}</small>
            <h3>{intent.title}</h3>
            <p>{intent.text}</p>
  ...
  return (
    <main className="c1-home">
      <section className={`c1-hero${heroMedia ? " has-image" : ""}`}>
        {heroMedia ? (
          <div className="c1-hero-image" style={{ backgroundImage: `url(${heroMedia.url})` }} role="img" aria-label={heroMedia.alt_text || "CHALIN ONE operations"} />
        ) : null}
        <div className="c1-hero-shade" aria-hidden="true" />
        <div className="c1-hero-signal-field" aria-hidden="true">
          <div className="c1-signal-core"><span>C1</span></div>
          <i className="c1-signal-ring ring-a" /><i className="c1-signal-ring ring-b" /><i className="c1-signal-ring ring-c" />
          <b className="c1-signal-word word-a">PARTS</b><b className="c1-signal-word word-b">MINING</b><b className="c1-signal-word word-c">EQUIPMENT</b>
        </div>
        <div className="c1-hero-copy">
          ...
          <div className="c1-hero-actions">
            <Link className="c1-button c1-button-gold" to="/businesses">Enter CHALIN ONE <b>↗</b></Link>
            <Link className="c1-button c1-button-ghost" to="/contact">Start with your need</Link>
          </div>
        </div>
        <div className="c1-hero-manifesto">
          <span>THE OPERATING UNIVERSE</span>
          <p>One company identity outside. Clear operating boundaries inside.</p>
          <div><b>03</b><small>specialist businesses</small></div>
        </div>
      ...
      {news.length > 0 ? (
        <section className="c1-newsline" aria-label="Latest company news">
          <div><span>LIVE</span><strong>CHALIN ONE SIGNAL</strong></div>
          <div className="c1-newsline-track">
            {news.slice(0, 4).map((item) => (
              <Link to={`/news/${item.slug}`} key={item.key || item.slug}>
                <b>{item.category?.name || "Update"}</b><span>{item.title}</span><time>{formatDate(item.published_at)}</time>
              </Link>
            ))}
          </div>
          <Link to="/news">All signals →</Link>
        </section>
      ) : null}
      ...
      {statistics.length > 0 ? (
        <section className="c1-stats" aria-label="Published company statistics">
          <div className="c1-stats-intro"><span>PUBLIC FACTS</span><strong>Numbers that have passed publication control.</strong></div>
      ...
      <section className="c1-section c1-business-section">
        <SectionHeading index="01" eyebrow="Business worlds" title="Three businesses. Three identities. One standard." text="Each CHALIN business gets a distinct world instead of being flattened into a generic corporate services list." action={<Link className="c1-text-link" to="/businesses">See the full group →</Link>} />
        <div className="c1-business-grid">
          {businessItems.map((business, index) => {
            ...
            return (
              <Link className={`c1-business-card world-${index + 1}`} to={`/businesses/${slug}`} key={business.key || slug}>
                {media?.url ? <CorporateMedia media={media} /> : <div className="c1-business-art"><b>{business.symbol || fallback.symbol}</b><span>{fallback.pulse}</span></div>}
      ...
      <section className="c1-operating-model">
        <div className="c1-operating-intro">
          <span>02 / THE CHALIN LOOP</span>
          <h2>Operate.<br />Control.<br />Deliver.<br />Communicate.</h2>
          <p>The public website is the visible edge of a much deeper operating model. Visitors see clarity; teams keep their proper business boundaries.</p>
        </div>
        <div className="c1-operating-panels">
          <article><span>01</span><strong>Operate</strong><p>Business-specific workspaces keep each operation focused.</p><i /></article>
          <article><span>02</span><strong>Control</strong><p>Governance connects responsibility, records and management oversight.</p><i /></article>
          <article><span>03</span><strong>Deliver</strong><p>Customer, equipment and field workflows move through clear operational paths.</p><i /></article>
          <article><span>04</span><strong>Communicate</strong><p>Only approved information crosses from operations into the public world.</p><i /></article>
        </div>
      </section>

      <section className="c1-section c1-project-section">
        <SectionHeading index="03" eyebrow="Field stories" title="Proof should feel bigger than a card grid." text="Projects become cinematic field stories when approved media exists; until then, the interface clearly shows what is waiting for publication." action={<Link className="c1-text-link" to="/projects">Project archive →</Link>} />
        {projects.length > 0 ? (
          <div className="c1-project-showcase">
            {projects.slice(0, 4).map((project, index) => (
              <Link className={`c1-project c1-project-${index + 1}`} to={`/projects/${project.slug}`} key={project.key || project.slug}>
                {project.media?.url ? <CorporateMedia media={project.media} /> : <div className="c1-project-fallback"><span>FIELD / {String(index + 1).padStart(2, "0")}</span></div>}
                <div className="c1-project-shade" />
                <div><span>{project.division?.name || humanize(project.status || "Project")}</span><h3>{project.title}</h3><p>{project.summary || project.location || project.location_text || ""}</p></div>
              </Link>
            ))}
          </div>
        ) : (
          <div className="c1-project-showcase c1-project-empty-showcase">
            {["PROJECT STORIES", "FIELD MEDIA", "DELIVERY PROOF"].map((label, index) => <article key={label}><span>0{index + 1}</span><strong>{label}</strong><p>Reserved for independently approved CHALIN ONE project publishing.</p></article>)}
          </div>
        )}
      </section>

      <section className="c1-equipment-band">
        <div className="c1-equipment-band-head"><div><span>04 / EQUIPMENT INTELLIGENCE</span><h2>Machines should be explored, not buried.</h2><p>A visual equipment stage ready for published availability, specifications and commercial information.</p></div><Link to="/equipment">Open equipment universe →</Link></div>
        {equipment.length > 0 ? (
          <div className="c1-equipment-strip">
            {equipment.slice(0, 5).map((item, index) => (
              <Link to={`/equipment/${item.slug}`} key={item.key || item.slug}>
                {item.media?.url ? <CorporateMedia media={item.media} /> : <div className="c1-equipment-fallback">EQ / {String(index + 1).padStart(2, "0")}</div>}
                <div><small>{item.category?.name || item.manufacturer || "Equipment"}</small><strong>{item.name}</strong><p>{item.short_description || [item.manufacturer, item.model].filter(Boolean).join(" ")}</p><span>{formatMoney(item.price) || humanize(item.availability || "Explore")}</span></div>
              </Link>
            ))}
          </div>
        ) : (
          <div className="c1-equipment-empty"><span>CATALOGUE SIGNAL OFFLINE</span><strong>Approved equipment will appear as an interactive visual catalogue.</strong><Link to="/contact?intent=hire">Make an equipment enquiry ↗</Link></div>
        )}
      </section>

      <section className="c1-leadership-feature">
        <div className="c1-leadership-image">
          {leadership[0] && (leadership[0].portrait || leadership[0].media)?.url ? <CorporateMedia media={leadership[0].portrait || leadership[0].media} /> : <div className="c1-leadership-monogram">C1</div>}
        </div>
        <div className="c1-leadership-copy">
          <span>05 / ACCOUNTABILITY</span>
          <h2>Responsibility should have a face.</h2>
          <p>Leadership is presented as accountability, not decoration. Only approved public profiles appear here.</p>
          {leadership[0] ? <div><strong>{leadership[0].full_name || leadership[0].name}</strong><small>{leadership[0].position || leadership[0].position_title || "Leadership"}</small></div> : <div><strong>Leadership publishing ready</strong><small>Awaiting approved profile content</small></div>}
          <Link className="c1-button c1-button-dark" to="/leadership">Meet leadership →</Link>
        </div>
      </section>
        ...
        <section className="c1-governed-sections">
          {page.sections.filter((section) => section.type !== "hero").map((section, index) => (
            <article className="c1-governed-block" key={section.key || index}>
              <div><span>GOVERNED CONTENT / {String(index + 1).padStart(2, "0")}</span>{section.heading ? <h2>{section.heading}</h2> : null}{section.subheading ? <p className="c1-governed-subtitle">{section.subheading}</p> : null}<StructuredContent value={section.content} /></div>
      ...
      <section className="c1-section c1-news-section">
        <SectionHeading index="06" eyebrow="Newsroom" title="A company should have a pulse." text="Announcements and stories move through an editorial newsroom instead of a generic list." action={<Link className="c1-text-link" to="/news">Enter newsroom →</Link>} />
        {news.length > 0 ? (
          <div className="c1-news-grid">
            {news.slice(0, 4).map((item, index) => (
              <Link to={`/news/${item.slug}`} className={index === 0 ? "is-featured" : ""} key={item.key || item.slug}>
                {item.media?.url ? <CorporateMedia media={item.media} /> : <div className="c1-news-fallback"><span>NEWS / {String(index + 1).padStart(2, "0")}</span></div>}
        ...
        ) : <EmptyState message="The newsroom is built and waiting for the first independently approved story." />}
      </section>

      <section className="c1-location-band">
        <div><span>07 / PHYSICAL WORLD</span><h2>Digital outside.<br />Real operations underneath.</h2><p>Published stores, offices, yards, sites and operating locations become part of the CHALIN ONE network.</p></div>
        <div className="c1-location-list">
          {locations.length > 0 ? locations.slice(0, 5).map((item, index) => (
            <article key={item.key || item.name}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{item.name}</strong><p>{[item.address, item.city, item.region, item.country].filter(Boolean).join(", ")}</p></div><b>⌖</b></article>
          )) : <article className="is-empty"><span>00</span><div><strong>Location network ready</strong><p>Approved public locations will appear here automatically.</p></div><b>⌖</b></article>}
        </div>
        <Link to="/locations">Open location network →</Link>
      </section>

      <section className="c1-final-stage">
        <div className="c1-final-stage-grid" aria-hidden="true" />
        <span>CHALIN 03 COMPANY LIMITED / CHALIN ONE</span>
        <h2>Don’t just visit.<br />Start something.</h2>
        <p>Find a part, discuss equipment, reach the mining team, explore a career or open a company conversation.</p>
        <div><Link className="c1-button c1-button-gold" to="/contact">Start with your need ↗</Link><Link className="c1-button c1-button-ghost" to="/businesses">Explore the group</Link></div>
      </section>
    </main>
  );
}
...
function PageHero({ eyebrow, title, text, media }) {
  return (
    <header className={`c1-page-hero${media?.url ? " has-media" : ""}`}>
      {media?.url ? <CorporateMedia media={media} eager /> : <div className="c1-page-hero-art" aria-hidden="true"><span>C1</span><i /><i /><i /></div>}
      <div className="c1-page-hero-shade" />
      <div className="c1-page-hero-copy"><span>{eyebrow}</span><h1>{title}</h1>{text ? <p>{text}</p> : null}</div>
      <div className="c1-page-hero-index">CHALIN ONE / {humanize(eyebrow)}</div>
    </header>
  );
}
...
function AboutPage() {
  const request = useRequest(async (signal) => {
    try { return await getPublicPage("about", { signal }); }
  ...
  return (
    <main className="c1-page">
      <PageHero eyebrow="About CHALIN ONE" title={page?.title || "One company. Built around real operations."} text={page?.summary || "CHALIN ONE is the public company platform connecting Chalin 03 Company Limited’s specialist operating businesses."} media={page?.media} />
      <section className="c1-editorial-grid"><div><span>WHO WE ARE</span><h2>One group identity. Clear operating boundaries.</h2></div><div><StructuredContent value={page?.body || "The group is designed so each business can operate with its own context while management, public communication and governance stay connected through one professional company platform."} /></div></section>
      <section className="c1-principles"><article><span>01</span><strong>Operational clarity</strong><p>Each business keeps the context, records and responsibilities that belong to its work.</p></article><article><span>02</span><strong>Professional governance</strong><p>Company information and management controls are structured for accountability and review.</p></article><article><span>03</span><strong>One public standard</strong><p>Customers, partners and visitors meet one coherent CHALIN ONE company experience.</p></article></section>
      {(page?.sections || []).map((section, index) => <section className="c1-content-split" key={section.key || index}><div><span>{section.type ? humanize(section.type) : "Company"}</span><h2>{section.heading || section.subheading}</h2><StructuredContent value={section.content} /></div><CorporateMedia media={section.primary_media || section.background_media} /></section>)}
    </main>
  );
}
...
function BusinessesPage() {
  ...
  return (
    <main className="c1-page">
      <PageHero eyebrow="Business worlds" title="Different missions deserve different worlds." text="Enter each specialist CHALIN business through its own visual and operational context." />
      <section className="c1-business-directory">
        {items.map((business, index) => {
          const fallback = BUSINESS_FALLBACKS[index] || BUSINESS_FALLBACKS[0];
          const slug = business.slug || fallback.slug;
          return <Link to={`/businesses/${slug}`} key={business.key || slug}><span>{fallback.number}</span>{business.media?.url ? <CorporateMedia media={business.media} /> : <div className="c1-directory-art">{fallback.symbol}</div>}<div><small>{business.label || fallback.label}</small><h2>{business.name}</h2><p>{business.short_description || business.description || fallback.description}</p><b>Enter business world →</b></div></Link>;
        })}
      </section>
      <IntentMatrix />
    </main>
...
function BusinessDetailPage() {
  ...
  const request = useRequest(async (signal) => {
    try { return await getPublicResource("divisions", slug, { signal }); }
    catch (error) { if (error?.response?.status === 404) return null; throw error; }
  }, [slug]);
  ...
  const intent = fallback === BUSINESS_FALLBACKS[0] ? "parts" : fallback === BUSINESS_FALLBACKS[1] ? "mining" : "hire";
  return (
    <main className="c1-page">
      <PageHero eyebrow="CHALIN ONE business" title={item.name} text={item.short_description || item.description} media={item.media} />
      <section className="c1-editorial-grid"><div><span>THE BUSINESS</span><h2>{fallback?.label || "Specialist operations"}</h2></div><div><StructuredContent value={item.body || item.details || item.description || fallback?.description} /></div></section>
      <section className="c1-business-signal"><span>{fallback?.symbol || "C1"}</span><div><small>BUSINESS SIGNAL</small><strong>{fallback?.pulse || "Specialist operations"}</strong><p>Move from information into the correct enquiry path without losing this business context.</p></div><Link className="c1-button c1-button-gold" to={`/contact?intent=${intent}`}>Start in this business ↗</Link></section>
      ...
      <section className="c1-business-cta"><div><span>WORK WITH {String(item.name || "CHALIN ONE").toUpperCase()}</span><h2>Turn interest into a clear next step.</h2></div><Link className="c1-button c1-button-gold" to={`/contact?intent=${intent}`}>Open the right enquiry →</Link></section>
    </main>
  );
}
  ...
  return (
    <main className="c1-page">
      ...
      <section className="c1-collection">
        {request.loading ? <LoadingState /> : request.error ? <ErrorState message={request.error} /> : items.length === 0 ? <EmptyState message={`The ${config.eyebrow.toLowerCase()} experience is ready for approved public content.`} /> : (
  ...
  return (
    <main className="c1-page">
      ...
      <section className="c1-detail-layout">
        <article><StructuredContent value={item.body || item.details || item.description} />{item.specifications ? <section><h2>Specifications</h2><StructuredContent value={item.specifications} /></section> : null}{item.features ? <section><h2>Features</h2><StructuredContent value={item.features} /></section> : null}<StructuredContent value={item.requirements} /><StructuredContent value={item.application_instructions || item.submission_instructions} /></article>
        <aside>
          ...
          <Link className="c1-button c1-button-dark" to="/contact">Continue with CHALIN ONE</Link>
        </aside>
      </section>
      {Array.isArray(item.gallery) && item.gallery.length > 0 ? <section className="c1-media-mosaic">{item.gallery.map((entry, index) => <CorporateMedia media={entry.media || entry} key={entry.asset_key || index} />)}</section> : null}
    </main>
...
function FaqPage() {
  ...
  return <main className="c1-page"><PageHero eyebrow="Help centre" title="Questions should disappear quickly." text="Published answers to common customer, supplier and company questions." /><section className="c1-faqs">{request.loading ? <LoadingState /> : request.error ? <ErrorState message={request.error} /> : items.length === 0 ? <EmptyState /> : items.map((item, index) => <details key={item.key || index}><summary><span>{String(index + 1).padStart(2, "0")}</span>{item.question}<b>+</b></summary><StructuredContent value={item.answer} /></details>)}</section></main>;
}
...
function ContactPage({ formSlug = "contact" }) {
  ...
  const formRequest = useRequest(async (signal) => {
    try { return await getPublicForm(formSlug, { signal }); }
    catch (error) { if (error?.response?.status === 404) return null; throw error; }
  }, [formSlug]);
  ...
  useEffect(() => {
    if (!form) return;
    const defaults = Object.fromEntries((form.fields || []).map((field) => [field.key, responseDefault(field)]));
    if (INTENT_TO_SERVICE[intent] && Object.prototype.hasOwnProperty.call(defaults, "service_interest")) defaults.service_interest = INTENT_TO_SERVICE[intent];
  ...
  return (
    <main className="c1-page">
      <PageHero eyebrow="Start something" title={selectedIntent ? selectedIntent.title : "Tell us what you need."} text={selectedIntent ? selectedIntent.text : "A structured entry point into the right CHALIN ONE business team."} />
      <section className="c1-contact-layout">
        <div className="c1-contact-context">
          <span>YOUR ENTRY SIGNAL</span>
          <h2>{selectedIntent ? selectedIntent.tag : "General company enquiry"}</h2>
          <p>The form stays governed, but the journey now carries your intent so you do not start from zero.</p>
          <div className="c1-contact-intents">{VISITOR_INTENTS.slice(0, 4).map((item) => <Link className={item.key === intent ? "is-active" : ""} to={`/contact?intent=${item.key}`} key={item.key}>{item.title}<span>↗</span></Link>)}</div>
          {locations.length > 0 ? <div className="c1-contact-locations"><strong>Published locations</strong>{locations.slice(0, 3).map((item) => <article key={item.key || item.name}><span>⌖</span><div><b>{item.name}</b><small>{[item.address, item.city, item.region, item.country].filter(Boolean).join(", ")}</small></div></article>)}</div> : null}
        </div>

        <div className="c1-contact-form-shell">
          {formRequest.loading ? <LoadingState label="Preparing your enquiry route…" /> : formRequest.error ? <ErrorState message={formRequest.error} /> : !form ? <EmptyState message="The public contact form has not been published yet." /> : success ? (
            <div className="c1-form-success"><span>ENQUIRY RECEIVED</span><h2>Your signal is in.</h2><p>{form.confirmation_message || "Thank you. Your enquiry has been received."}</p><strong>Reference: {success.reference_code || "Provided by the enquiry desk"}</strong><Link className="c1-button c1-button-dark" to="/">Return to CHALIN ONE</Link></div>
          ) : (
            <form className="c1-contact-form" onSubmit={submit}>
              <div className="c1-form-heading"><span>CHALIN ONE / ENQUIRY DESK</span><h2>{form.name}</h2><p>{form.description}</p></div>
              <div className="c1-contact-basics">
                <label><span>Your name</span><input required value={contact.full_name} onChange={(event) => setContact((value) => ({ ...value, full_name: event.target.value }))} /></label>
                <label><span>Company</span><input value={contact.company_name} onChange={(event) => setContact((value) => ({ ...value, company_name: event.target.value }))} /></label>
                <label><span>Email</span><input type="email" value={contact.email} onChange={(event) => setContact((value) => ({ ...value, email: event.target.value }))} /></label>
                <label><span>Phone</span><input type="tel" value={contact.phone} onChange={(event) => setContact((value) => ({ ...value, phone: event.target.value }))} /></label>
                <label className="c1-honeypot" aria-hidden="true"><span>Website</span><input tabIndex="-1" autoComplete="off" value={contact.website} onChange={(event) => setContact((value) => ({ ...value, website: event.target.value }))} /></label>
              </div>
              <div className="c1-dynamic-fields">{(form.fields || []).map((field) => <label key={field.key}><span>{field.label}{field.required ? " *" : ""}</span>{field.help_text ? <small>{field.help_text}</small> : null}<PublicField field={field} value={responses[field.key]} onChange={(value) => setResponses((current) => ({ ...current, [field.key]: value }))} /></label>)}</div>
              <label className="c1-consent"><input type="checkbox" required={form.settings?.require_consent !== false} checked={contact.consent_given} onChange={(event) => setContact((value) => ({ ...value, consent_given: event.target.checked }))} /><span>I agree that the information provided can be used to review and respond to this enquiry.</span></label>
              {submitError ? <div className="c1-inline-error" role="alert">{submitError}</div> : null}
              <button className="c1-button c1-button-gold" type="submit" disabled={submitting}>{submitting ? "Routing your enquiry…" : form.settings?.submit_label || "Send enquiry"}<b>↗</b></button>
            </form>
          )}
        </div>
      </section>
...
function PublishedPage() {
  ...
  return <main className="c1-page"><PageHero eyebrow={page.menu_title || "CHALIN ONE"} title={page.title} text={page.subtitle || page.summary} media={page.media} /><section className="c1-published-body"><StructuredContent value={page.body} />{(page.sections || []).map((section, index) => <section className="c1-content-split" key={section.key || index}><div><span>{section.type ? humanize(section.type) : `Section ${index + 1}`}</span><h2>{section.heading}</h2>{section.subheading ? <p>{section.subheading}</p> : null}<StructuredContent value={section.content} /></div><CorporateMedia media={section.primary_media || section.background_media} /></section>)}</section></main>;
}

function NotFoundPage() {
  useMetadata("Not found | CHALIN ONE", "This CHALIN ONE public page is not available.");
  return <main className="c1-not-found"><div><span>404 / SIGNAL LOST</span><h1>You’ve moved beyond the published map.</h1><p>This page is not available in the public CHALIN ONE experience.</p><div><Link className="c1-button c1-button-gold" to="/">Return home</Link><Link className="c1-button c1-button-ghost" to="/contact">Ask CHALIN ONE</Link></div></div><b>C1</b></main>;
}

function LegacyWebsiteRedirect() {
  const location = useLocation();
  const suffix = location.pathname.replace(/^\/website\/?/, "");
  return <Navigate replace to={suffix ? `/${suffix}${location.search}${location.hash}` : `/${location.search}${location.hash}`} />;
}

export function PublicCorporateWebsiteUnavailable() {
  return <main className="c1-unavailable"><img src="/chalin03-logo.png" alt="Chalin 03 Company Limited" /><span>CHALIN ONE</span><h1>Public website temporarily unavailable.</h1><p>The staff system remains separate and secure.</p><Link className="c1-button c1-button-dark" to="/login">Open Staff portal</Link></main>;
}
