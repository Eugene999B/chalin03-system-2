...
export default function ContentStudioWebsiteControlCenter({ onOpenSection }) {
  ...
  return <div className="cs-wcc-shell">
    ...
    {tab === "redirects" ? <section className="cs-wcc-queue"><header><div><span>REDIRECT INTELLIGENCE</span><h3>{data.redirectCandidates.length} review candidates</h3><p>Advisory only. No redirect rule is written by this manager.</p></div></header>{data.redirectCandidates.length ? <div className="cs-wcc-simple-grid">{data.redirectCandidates.map((item, index) => <article key={`${item.kind}-${index}`}><span>{String(item.kind).replaceAll("_", " ")}</span><div className="cs-wcc-route"><strong>{item.source || "Unknown"}</strong><b>→</b><strong>{item.destination || "Confirm destination"}</strong></div><p>{item.note}</p><button type="button" onClick={() => onOpenSection?.(item.navigation_id ? "navigation" : "pages")}>Open owning manager →</button></article>)}</div> : <Empty title="No redirect candidates detected." message="Current page canonicals and governed internal navigation do not suggest a redirect review." />}</section> : null}

    <footer className="cs-wcc-note"><strong>Read-only control plane</strong><span>Fixes still happen through governed Pages or Navigation drafts and the normal review/publish workflow.</span></footer>
  </div>;
}
