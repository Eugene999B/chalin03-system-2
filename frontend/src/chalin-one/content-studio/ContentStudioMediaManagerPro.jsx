  ...
  return (
    <div className="cs-media-pro-shell">
      ...
      <section className="cs-media-pro-kpis">
        ...
        <Kpi label="UNCATEGORIZED" value={summary.uncategorized} note="assets outside folders" onClick={() => quickFilter({ folder_id: "uncategorized" })} />
      </section>
      ...
      {tab === "library" ? (
        <>
          <form className="cs-media-pro-filterbar" onSubmit={applyFilters}>
            ...
            <select value={filterForm.folder_id} onChange={(event) => setFilterForm((current) => ({ ...current, folder_id: event.target.value }))}><option value="">All folders</option><option value="uncategorized">Uncategorized</option>{folderOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>
