...
const CHAT_STARTERS = Object.freeze({
  copilot: Object.freeze([
    "Summarize the approved procedures relevant to this workspace.",
    "What information is available in the approved knowledge base?",
    "Explain my active workspace and location scope.",
    "Show the effective CHALIN ONE intelligence feature status.",
  ]),
  executive: Object.freeze([
    "Summarize the approved group knowledge currently available.",
    "Separate the available facts, assumptions and evidence gaps.",
    "What approved executive knowledge is available?",
    "Explain the current intelligence safety and execution boundaries.",
  ]),
});
    ...
    <>
      <div className="ci-feedback" aria-label="Rate this answer">
        ...
        <button
          type="button"
          title="Unsafe"
          aria-label="Report unsafe answer"
          disabled={Boolean(sent)}
      ...
      <div className="ci-message-body">
        ...
        {assistant ? (
          <div className="ci-message-meta">
            ...
            {resultMeta?.usage ? (
              <span>
                Tokens: {Number(resultMeta.usage.input_tokens || 0) + Number(resultMeta.usage.output_tokens || 0)}
              </span>
            ) : null}
          </div>
        ) : null}
    ...
    <section className="ci-chat-panel">
      <div className="ci-panel ci-chat-stream" ref={streamRef}>
        {messages.length === 0 ? (
          <div className="ci-empty-chat">
            <div>
              <span className="ci-eyebrow">Governed assistance</span>
              <h1>{persona === "executive" ? "Chalin Executive" : "Chalin Copilot"}</h1>
              <p>
                Answers use only approved knowledge and registered tools allowed for your active workspace. Sensitive changes remain human-controlled.
              </p>
        ...
        {sending ? (
          <div className="ci-state" role="status">
            Reviewing permissions, approved evidence and safe tools…
          </div>
        ) : null}
      ...
      <div className="ci-composer-wrap">
        <form className="ci-composer" onSubmit={submit}>
          ...
          >
            {sending ? "Reviewing…" : "Send securely"}
          </button>
        </form>
        <p className="ci-composer-note">
          Do not enter passwords, provider keys or unnecessary identity documents. AI cannot approve or execute sensitive changes.
        </p>
      </div>
    </section>
  );
    ...
    <div className="ci-modal-backdrop" role="presentation" onMouseDown={onClose}>
      ...
      >
        <header className="ci-modal-head">
          ...
          <button
            type="button"
            className="ci-button ci-button-secondary ci-icon-button"
          ...
          <form className="ci-form" onSubmit={onSubmit}>
            ...
            <div className="ci-form-grid">
              <label>
                ...
                <input
                  ...
                  placeholder="equipment_hire_release_policy"
                  onChange={(event) =>
              ...
              <label>
                ...
                <input
                  ...
                  placeholder="Policy register or approved file reference"
                  onChange={(event) =>
                    setForm((current) => ({ ...current, source_reference: event.target.value }))
                  }
                />
              </label>
              <label>
            ...
            <label>
              ...
              <textarea
                ...
                placeholder="Enter factual, source-backed knowledge. This remains a draft until independent review and publication."
                onChange={(event) =>
                  setForm((current) => ({ ...current, body_text: event.target.value }))
                }
              />
            </label>
            <button className="ci-button ci-button-primary" type="submit" disabled={saving}>
              {saving ? "Creating draft…" : "Create governed draft"}
            </button>
          </form>
        </div>
      </section>
...
function KnowledgeDetailModal({
  ...
  const pending = details?.approvals?.find(
  ...
  return (
    <div className="ci-modal-backdrop" role="presentation" onMouseDown={onClose}>
      ...
      >
        <header className="ci-modal-head">
          <div>
            <span className="ci-eyebrow">Governed knowledge</span>
            <h2 id="knowledge-detail-title">{details.source.title}</h2>
          </div>
          <button
            type="button"
            className="ci-button ci-button-secondary ci-icon-button"
        ...
        <div className="ci-modal-body">
          ...
          <div className="ci-card">
            ...
            <div className="ci-card-meta">
              ...
              {details.source.owner_workspace_code ? (
                <span>{humanize(details.source.owner_workspace_code)}</span>
              ) : null}
            </div>
          </div>
          {latest ? (
            <article className="ci-card" style={{ marginTop: 14 }}>
              <span className="ci-eyebrow">Latest version {latest.version_number}</span>
              <h3>{latest.title}</h3>
              <p style={{ whiteSpace: "pre-wrap" }}>{latest.body_text}</p>
              <div className="ci-card-meta">
                <span>{humanize(latest.version_status)}</span>
                <span>Checksum {String(latest.checksum_sha256 || "").slice(0, 12)}…</span>
                {latest.published_at ? <span>Published {formatDate(latest.published_at, true)}</span> : null}
              </div>
            </article>
          ) : null}
          {pending ? (
            <article className="ci-card" style={{ marginTop: 14 }}>
              <span className="ci-eyebrow">Pending review</span>
              <p>{pending.request_note || "No review note supplied."}</p>
              <div className="ci-card-meta">
                <span>Assigned user {pending.assigned_to || "Unassigned"}</span>
                <span>Approval #{pending.id}</span>
              </div>
            </article>
          ) : null}
          <div className="ci-form" style={{ marginTop: 14 }}>
            ...
            {permissions.has("ai.knowledge.manage") && latest?.version_status === "draft" ? (
              <label>
                Independent reviewer user ID
                <input
                  type="number"
                  min="1"
                  value={reviewerId}
            ...
            <div className="ci-card-actions">
              {permissions.has("ai.knowledge.manage") && latest?.version_status === "draft" ? (
                <button
                  ...
                  onClick={() => onSubmitReview(latest.id, reviewerId, note)}
                >
                  Submit exact version
                </button>
              ) : null}
              {permissions.has("ai.knowledge.review") && pending ? (
                <>
                  <button
                    type="button"
                    className="ci-button ci-button-primary"
                    disabled={busy}
                    onClick={() => onDecision(pending.id, "approved", note)}
                  >
                    Approve
                  </button>
                  <button
                    ...
                    disabled={busy}
                    onClick={() => onDecision(pending.id, "rejected", note)}
                  >
                    Reject
                  </button>
                </>
              ) : null}
              {permissions.has("ai.knowledge.publish") && latest?.version_status === "approved" ? (
                <button
                  type="button"
                  className="ci-button ci-button-primary"
                  disabled={busy}
                  onClick={() => onPublish(latest.id)}
                >
...
function KnowledgePanel({ permissions }) {
  ...
  const load = useCallback(async (signal) => {
    ...
    setError("");
    try {
      setItems(await listAiKnowledge({}, { signal }));
    } catch (requestError) {
  ...
  async function openDetails(sourceId) {
    setDetailError("");
    try {
      setSelected(await getAiKnowledgeSource(sourceId));
    } catch (requestError) {
      setDetailError(aiErrorMessage(requestError));
    }
  }

  async function refreshSelected() {
    if (!selected?.source?.id) return;
    setSelected(await getAiKnowledgeSource(selected.source.id));
    await load();
  }
  ...
  return (
    <section className="ci-page">
      <header className="ci-page-heading">
        <div>
          <span className="ci-eyebrow">Approved evidence only</span>
          <h1>Knowledge governance</h1>
          <p>
            Draft, independently review and publish exact knowledge versions used by CHALIN ONE intelligence. Draft and rejected content is never retrieved as evidence.
          </p>
        </div>
        {permissions.has("ai.knowledge.manage") ? (
          <button
            className="ci-button ci-button-primary"
            type="button"
        ...
        <div className="ci-card-grid">
          {items.map((item) => (
            <article className="ci-card" key={item.source_key}>
              <span className="ci-eyebrow">{humanize(item.source_type)}</span>
              <h2>{item.title}</h2>
              <p>{item.description || "No source description supplied."}</p>
              <div className="ci-card-meta">
                <span>{humanize(item.visibility)}</span>
                <span>{humanize(item.source_status)}</span>
                {item.latest_version_status ? <span>Latest: {humanize(item.latest_version_status)}</span> : null}
                {item.owner_workspace_code ? <span>{humanize(item.owner_workspace_code)}</span> : null}
              </div>
              <div className="ci-card-actions">
                <button
                  type="button"
      ...
      {selected ? (
        <KnowledgeDetailModal
          ...
          onSubmitReview={(versionId, reviewerId, note) =>
            workflow(() =>
              submitAiKnowledgeVersion(selected.source.id, versionId, {
          ...
          onDecision={(approvalId, decision, note) =>
            workflow(() => decideAiKnowledgeApproval(approvalId, decision, note))
          }
          onPublish={(versionId) =>
            workflow(() => publishAiKnowledgeVersion(selected.source.id, versionId))
          }
        />
      ) : null}
    </section>
...
function UsagePanel() {
  ...
  useEffect(() => {
    const controller = new AbortController();
    listAiUsage({ days: 30 }, { signal: controller.signal })
      .then(setRows)
      .catch((requestError) => {
    ...
    () =>
      rows.reduce(
        (summary, row) => ({
          ...
          tokens: summary.tokens + Number(row.total_tokens || 0),
          cost: summary.cost + Number(row.cost_micros || 0),
        }),
  ...
  return (
    <section className="ci-page">
      <header className="ci-page-heading">
        <div>
          <span className="ci-eyebrow">Cost and safety visibility</span>
          <h1>Usage ledger</h1>
          <p>
            Review scoped provider requests, token totals and integer-micro cost evidence. Provider secrets are never exposed here.
          </p>
        </div>
      </header>
      <div className="ci-card-grid" style={{ marginBottom: 18 }}>
        <article className="ci-card"><span className="ci-eyebrow">Requests</span><h2>{totals.requests.toLocaleString("en-GH")}</h2></article>
        <article className="ci-card"><span className="ci-eyebrow">Tokens</span><h2>{totals.tokens.toLocaleString("en-GH")}</h2></article>
        <article className="ci-card"><span className="ci-eyebrow">Cost micros</span><h2>{totals.cost.toLocaleString("en-GH")}</h2></article>
      </div>
      <StatePanel loading={loading} error={error} empty={!loading && !error && rows.length === 0}>
        <div className="ci-usage-table">
          <table>
            <thead>
              <tr>
                <th>Date</th>
            ...
            <tbody>
              {rows.map((row, index) => (
                <tr key={`${row.usage_date}-${row.provider_key}-${row.model_key}-${index}`}>
                  <td>{formatDate(row.usage_date)}</td>
                  <td>{row.provider_key}</td>
                  <td>{row.model_key}</td>
                  <td>{Number(row.request_count || 0).toLocaleString("en-GH")}</td>
                  <td>{Number(row.input_tokens || 0).toLocaleString("en-GH")}</td>
                  <td>{Number(row.output_tokens || 0).toLocaleString("en-GH")}</td>
                  <td>{Number(row.cost_micros || 0).toLocaleString("en-GH")}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
export default function ChalinIntelligenceWorkspace() {
  ...
  useEffect(() => {
    ...
    getAiStatus({ signal: controller.signal })
      .then((result) => {
        setStatus(result);
        if (!canUsePersona(result, AI_PERSONAS.copilot) && canUsePersona(result, AI_PERSONAS.executive)) {
          setPersona(AI_PERSONAS.executive);
        }
      })
      .catch((error) => {
        if (!controller.signal.aborted) setStatusError(aiErrorMessage(error));
      })
      .finally(() => {
    ...
    async (signal) => {
      ...
      try {
        const [conversationRows, toolRows] = await Promise.all([
          listAiConversations(persona, {}, { signal }),
          listAiTools(persona, { signal }),
        ]);
  ...
  async function send(message) {
    ...
    try {
      const result = await sendAiMessage(persona, {
      ...
      const assistant = {
        ...
        model_key: result.provider?.model,
  ...
  async function archiveCurrent() {
    if (!conversation?.key) return;
    try {
      await archiveAiConversation(persona, conversation.key);
      newConversation();
      await loadConversations();
    } catch (error) {
      setConversationError(aiErrorMessage(error));
  ...
  if (statusLoading) {
    return (
      <main className="ci-shell" style={{ display: "grid", placeItems: "center" }}>
        <StatePanel loading />
      </main>
    );
  }
  ...
  if (statusError || !status) {
    return (
      <main className="ci-shell" style={{ display: "grid", placeItems: "center", padding: 20 }}>
        <div>
          <StatePanel error={statusError || "The intelligence status could not be verified."} />
          <p style={{ textAlign: "center" }}><Link to="/">Return to the staff system</Link></p>
        </div>
      </main>
    );
  }
      ...
      <header className="ci-topbar">
        <div className="ci-brand">
          ...
          <span className="ci-brand-copy">
            <strong>CHALIN ONE Intelligence</strong>
            <small>{user?.full_name || user?.username || "Authorized staff"}</small>
          </span>
        </div>
        <div className="ci-topbar-actions">
          <span className="ci-scope-pill">{workspaceName || humanize(workspaceCode)}</span>
          <span className="ci-provider-pill" data-active={providerActive ? "true" : "false"}>
            Provider: {status.provider?.key || "disabled"}
          </span>
          <span
            className="ci-status-pill"
            data-state={status.ai_actions_enabled ? "danger" : "success"}
          >
            {status.ai_actions_enabled ? "Approved actions enabled" : "Read / recommend / prepare only"}
          </span>
          <Link className="ci-button ci-button-secondary" to="/">Staff system</Link>
        </div>
      </header>
      {!providerActive ? (
        <div className="ci-banner" role="status" style={{ marginInline: 16 }}>
          The provider is safely disabled. The workspace, permissions, knowledge governance and audit foundation remain available for controlled testing.
        </div>
      ) : null}
      <div className="ci-workspace">
        <aside className="ci-sidebar">
          <div className="ci-sidebar-head">
            <button
              type="button"
              className="ci-button ci-button-primary ci-new-chat"
            ...
            <div className="ci-persona-switch" aria-label="Intelligence persona">
              <button
                type="button"
                aria-pressed={persona === AI_PERSONAS.copilot}
                disabled={!canUsePersona(status, AI_PERSONAS.copilot)}
                onClick={() => setPersona(AI_PERSONAS.copilot)}
              >
                Copilot
              </button>
              <button
                type="button"
                aria-pressed={persona === AI_PERSONAS.executive}
                disabled={!canUsePersona(status, AI_PERSONAS.executive)}
                onClick={() => setPersona(AI_PERSONAS.executive)}
              >
                Executive
              </button>
            </div>
          ...
          <div className="ci-sidebar-section">
            ...
            {conversationLoading ? (
              <div className="ci-state">Loading conversations…</div>
            ) : (
              <div className="ci-conversation-list">
                {conversations.map((item) => (
                  <button
                    type="button"
                    className="ci-conversation-item"
                    key={item.key}
                    aria-current={conversation?.key === item.key ? "true" : undefined}
                ...
                {conversations.length === 0 ? (
                  <div className="ci-state"><span>No saved conversations.</span></div>
                ) : null}
              </div>
            )}
          </div>
          <div className="ci-sidebar-footer">
            <span className="ci-sidebar-label">Registered tools</span>
            <p style={{ fontSize: ".72rem", color: "var(--ci-slate-700)" }}>
              {tools.length} permission-scoped tool{tools.length === 1 ? "" : "s"} available.
            </p>
            {conversation?.key ? (
              <div className="ci-card-actions">
                <button type="button" className="ci-button ci-button-secondary" onClick={renameCurrent}>Rename</button>
                <button type="button" className="ci-button ci-button-danger" onClick={archiveCurrent}>Archive</button>
              </div>
            ) : null}
          </div>
        </aside>
            ...
            {tab === "chat" ? (
              personaAvailable ? (
                ...
                <div className="ci-page">
                  <StatePanel error={`The ${humanize(persona)} persona is disabled or not granted to this account.`} />
                </div>
              )
            ) : null}
            {tab === "knowledge" && knowledgeAvailable ? (
              <KnowledgePanel permissions={permissions} />
            ) : null}
