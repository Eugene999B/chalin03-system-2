...
export default function AuditSignoffHistoryPage() {
  ...
  const summary = useMemo(() => {
    ...
    const approved = filteredSignoffs.filter((item) => item.period_status === "approved").length;
    const reviewed = filteredSignoffs.filter((item) => item.period_status === "reviewed").length;
    const draft = filteredSignoffs.filter((item) => item.period_status === "draft").length;
    const rejected = filteredSignoffs.filter((item) => item.period_status === "rejected").length;
    const averageScore = total > 0
      ? Math.round(filteredSignoffs.reduce((sum, item) => sum + Number(item.audit_score || 0), 0) / total)
      : 0;
  ...
  async function loadSignoffs() {
    ...
    try {
      ...
      setError(requestError.response?.data?.message || "Something went wrong while fetching audit sign-offs.");
    } finally {
      setLoading(false);
    }
  }
  ...
  function exportHistoryCsv() {
    ...
    downloadTextFile("chalin03_audit_signoff_history.csv", makeCsv(rows), "text/csv;charset=utf-8");
  ...
  function downloadCertificateWord(signoff) {
    ...
    downloadTextFile(`chalin03_signoff_certificate_${signoff.id}.doc`, wordDocument, "application/msword;charset=utf-8");
    setMessage("Sign-off certificate Word document downloaded.");
  }

  async function deleteSignoff(signoff) {
    const confirmed = window.confirm(`Delete audit sign-off for "${signoff.period_label}"?`);
    if (!confirmed) return;
    ...
    try {
      ...
      setError(requestError.response?.data?.message || "Failed to delete audit sign-off.");
    }
  }
    ...
    <div style={styles.page}>
      <div style={styles.hero}>
        <div>
          ...
          <p style={styles.subtitle}>View saved accounting approvals, approved periods, audit scores and sign-off records.</p>
        </div>
        <div style={styles.heroActions}>
          <button type="button" onClick={loadSignoffs} disabled={loading}>{loading ? "Loading..." : "Refresh"}</button>
          <button type="button" className="secondary-button" onClick={exportHistoryCsv}>Export History CSV</button>
        </div>
      </div>
      ...
      <div style={styles.filterPanel}>
        <label>
          Search
          <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search period or names" />
        </label>
        <label>
          Period Type
        ...
        <label>
          Status
          <select value={periodStatus} onChange={(event) => setPeriodStatus(event.target.value)}>
      ...
      <div style={styles.panel}>
        ...
        {loading ? (
          ...
          <div style={styles.emptyState}>No audit sign-offs found.</div>
        ) : (
          <div style={styles.tableWrap}>
            <table style={styles.table}>
              <thead>
                <tr>
                  <th>ID</th><th>Period</th><th>Score</th><th>Status</th><th>Prepared</th><th>Reviewed</th><th>Approved</th><th>Review Date</th><th>Saved By</th><th>Updated</th><th>Actions</th>
                </tr>
              </thead>
              <tbody>
                {filteredSignoffs.map((item) => (
                  <tr key={item.id}>
                    <td>#{item.id}</td>
                    <td><strong>{item.period_label}</strong><br /><small>{item.period_type}</small></td>
                    <td><span style={{ ...styles.badge, ...getScoreStyle(item.audit_score) }}>{Number(item.audit_score || 0)}%</span></td>
                    <td><span style={{ ...styles.badge, ...getStatusStyle(item.period_status) }}>{getStatusLabel(item.period_status)}</span></td>
                    ...
                    <td>
                      <div style={styles.actionButtons}>
                        <button type="button" className="secondary-button" onClick={() => printCertificate(item)}>Print</button>
                        <button type="button" className="secondary-button" onClick={() => downloadCertificateWord(item)}>Word</button>
                        <button type="button" style={styles.deleteButton} onClick={() => deleteSignoff(item)}>Delete</button>
                      </div>
                    </td>
                  </tr>
                ))}
...
const styles = {
  page: { width: "100%", maxWidth: "1680px", margin: "0 auto", paddingBottom: "40px" },
  hero: {
    display: "flex", justifyContent: "space-between", gap: "18px", flexWrap: "wrap", alignItems: "flex-start", marginBottom: "18px", padding: "24px", borderRadius: "26px",
    background: "linear-gradient(135deg, #07182c 0%, #0d2f55 55%, #111827 100%)", color: "#ffffff", boxShadow: "0 22px 55px rgba(7, 24, 44, 0.24)",
  },
  eyebrow: { margin: 0, color: "#e0ba28", fontSize: "12px", fontWeight: "950", letterSpacing: "0.08em", textTransform: "uppercase" },
  title: { margin: "6px 0 0", fontSize: "clamp(28px, 4vw, 44px)", fontWeight: "950", lineHeight: 1.05 },
  subtitle: { margin: "10px 0 0", color: "rgba(255,255,255,0.76)", maxWidth: "850px", lineHeight: 1.6 },
  ...
  summaryCard: { padding: "16px", borderRadius: "20px", background: "#ffffff", border: "1px solid #e2e8f0", boxShadow: "0 14px 30px rgba(15,23,42,0.07)" },
  filterPanel: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "12px", padding: "18px", borderRadius: "22px", background: "#ffffff", border: "1px solid #e2e8f0", boxShadow: "0 18px 40px rgba(15,23,42,0.08)", marginBottom: "18px" },
  panel: { background: "#ffffff", borderRadius: "22px", padding: "20px", border: "1px solid #e2e8f0", boxShadow: "0 18px 40px rgba(15,23,42,0.08)", minWidth: 0 },
  tableWrap: { width: "100%", overflowX: "auto", marginTop: "12px" },
  table: { width: "100%", borderCollapse: "collapse", minWidth: "1100px" },
  badge: { display: "inline-flex", padding: "6px 10px", borderRadius: "999px", fontWeight: "950", fontSize: "12px" },
  actionButtons: { display: "flex", flexWrap: "wrap", gap: "7px" },
  deleteButton: { background: "#dc2626", color: "#ffffff", border: "none", borderRadius: "10px", padding: "8px 10px", fontWeight: "900", cursor: "pointer" },
  emptyState: { padding: "18px", borderRadius: "16px", background: "#f8fafc", color: "#64748b", border: "1px dashed #cbd5e1", textAlign: "center", fontWeight: "800" },
};
