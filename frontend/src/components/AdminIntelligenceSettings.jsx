...
function rangeFor(type) { const now = new Date(); if (type === "weekly") { const start = new Date(now); start.setDate(now.getDate() - ((now.getDay() + 6) % 7)); return { from: start.toISOString().slice(0, 10), to: now.toISOString().slice(0, 10) }; } return { from: new Date(now.getFullYear(), now.getMonth(), 1).toISOString().slice(0, 10), to: now.toISOString().slice(0, 10) }; }
function rolePurpose(role) { if (role === "admin") return "Full business picture: profitability, cash, customer debt, stock position, installment portfolio, arrears and management advice."; if (role === "manager") return "Action-focused view: sales performance, stock pressure, overdue debt, installment collection and operational priorities."; return "Control-focused view: Daily Closing evidence, variances, debt ageing, installment arrears, payment evidence and audit follow-up."; }
function reportContents(role, period) { const prefix = period === "Weekly" ? "Saturday weekly closing" : "month-end closing"; if (role === "admin") return `${prefix}: complete Spare Parts + Installment business review, profitability, money received, debt exposure, stock risk, installment arrears and clear advice.`; if (role === "manager") return `${prefix}: sales and margin, replenishment needs, overdue customers, installment collections and concrete operational priorities.`; return `${prefix}: verified closings, shortages/variances, changed-after-close activity, debt exceptions, installment payment evidence and audit actions.`; }
function buildManualMessage(type, role, summary, accounts) { const spare = summary?.spare_parts || {}; const cash = summary?.cash_control || {}; const installmentOutstanding = accounts.reduce((sum, item) => sum + Number(item.outstanding_balance || 0), 0); const installmentOverdue = accounts.reduce((sum, item) => sum + Number(item.overdue_amount || 0), 0); const installmentPaid = accounts.reduce((sum, item) => sum + Number(item.amount_paid || 0), 0); const installmentArrears = accounts.filter((item) => Number(item.overdue_amount || 0) > 0.01).length; const title = type === "weekly" ? "Weekly Business Intelligence" : "Monthly Business Intelligence"; if (role === "auditor") return `Chalin 03 ${title}. Daily Closings: ${cash.closing_count || 0}; verified ${cash.verified_count || 0}; awaiting verification ${cash.awaiting_verification_count || 0}; variance exposure ${money(cash.absolute_variance)}; changed after close ${cash.changed_after_close_count || 0}. Customer debt ${money(spare.debt_balance)}. Installment portfolio: ${accounts.length} account(s), paid ${money(installmentPaid)}, outstanding ${money(installmentOutstanding)}, overdue ${money(installmentOverdue)} across ${installmentArrears} account(s). Review evidence, payment allocations and outstanding control exceptions.`; if (role === "manager") return `Chalin 03 ${title}. Sales ${money(spare.sales_total)}, received ${money(spare.sales_received)}, expenses ${money(spare.expenses_total)}, operating result ${money(spare.estimated_store_margin)}. Low-stock products ${spare.low_stock_count || 0}. Customer debt ${money(spare.debt_balance)}. Installment collections ${money(installmentPaid)}, outstanding ${money(installmentOutstanding)}, overdue ${money(installmentOverdue)}. Priority: replenish risk stock, collect ageing debt and act on installment arrears.`; return `Chalin 03 ${title}. Sales ${money(spare.sales_total)}, received ${money(spare.sales_received)}, expenses ${money(spare.expenses_total)}, estimated operating result ${money(spare.estimated_store_margin)}. Customer debt ${money(spare.debt_balance)}. Spare Parts low-stock ${spare.low_stock_count || 0}. Installment accounts ${accounts.length}; collections ${money(installmentPaid)}; outstanding ${money(installmentOutstanding)}; overdue ${money(installmentOverdue)} across ${installmentArrears} account(s). Daily Closing variance exposure ${money(cash.absolute_variance)}. Management should review debt, stock, installment arrears and cash exceptions.`; }

export default function AdminIntelligenceSettings() {
  const { user, canAccessAllBranches } = useAuth();
  const [open, setOpen] = useState(false); const [rules, setRules] = useState([]); const [loading, setLoading] = useState(false); const [savingId, setSavingId] = useState(null); const [sending, setSending] = useState(""); const [notice, setNotice] = useState(""); const [error, setError] = useState("");
  ...
  async function loadRules() { setLoading(true); setError(""); try { const response = await axiosClient.get("/notifications/rules"); setRules(response.data?.rules || []); } catch (e) { setError(e.response?.data?.message || "Intelligence controls could not be loaded."); } finally { setLoading(false); } }
  useEffect(() => { if (open) loadRules(); }, [open]);
  async function updateRule(rule, changes) { setSavingId(rule.id); setError(""); setNotice(""); setRules((current) => current.map((item) => item.id === rule.id ? { ...item, ...changes } : item)); try { await axiosClient.patch(`/notifications/rules/${rule.id}`, changes); setNotice("Intelligence setting saved."); } catch (e) { setError(e.response?.data?.message || "The intelligence setting could not be saved."); await loadRules(); } finally { setSavingId(null); } }
  async function runMonitoringNow() { setError(""); setNotice(""); try { await axiosClient.post("/notifications/sync", { workspace_code: "spare_parts" }); setNotice("Spare Parts and Installment monitoring was refreshed. No SMS was sent."); await loadRules(); } catch (e) { setError(e.response?.data?.message || "The monitoring refresh could not be started."); } }
  async function sendReportNow(type) {
    setSending(type); setError(""); setNotice("");
    try {
      const range = rangeFor(type);
      const [summaryResponse, accountsResponse, usersResponse, settingsResponse] = await Promise.all([
      ...
      const summary = summaryResponse.data?.summary || {}; const accounts = accountsResponse.data?.accounts || []; const users = usersResponse.data?.users || []; const settings = settingsResponse.data?.settings || {};
      const periodRules = reportRules.filter((rule) => reportMeta(rule.rule_code).period.toLowerCase() === type);
      let sent = 0; const failures = []; const destinations = new Map();
      for (const role of ["admin", "manager", "auditor"]) {
        ...
        for (const account of users.filter((item) => item.is_active && item.role === role && String(item.phone || "").trim())) {
          const phone = String(account.phone).trim(); if (!destinations.has(phone)) destinations.set(phone, { phone, message, role });
        }
      }
