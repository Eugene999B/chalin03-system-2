...
export default function CustomerFeatureControlsPanel() {
  ...
  async function load() {
    ...
    try {
      ...
      setError(requestError.response?.data?.message || "Could not load customer feature controls.");
  ...
  return (
    <section style={{ margin: "0 0 18px", padding: 20, border: "1px solid #dbe3ef", borderRadius: 24, background: "linear-gradient(135deg,#fff,#f8fafc)", boxShadow: "0 18px 45px rgba(15,23,42,.07)" }}>
      <div style={{ marginBottom: 14 }}>
        ...
        <p style={{ margin: "7px 0 0", color: "#64748b", lineHeight: 1.5, fontWeight: 650 }}>Control whether customer identity editing and duplicate-customer merging are available across Spare Parts. The server enforces both switches too.</p>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2,minmax(0,1fr))", gap: 12 }}>
        {[
          ["customer_identity_editing_enabled", "Customer identity editing", "Controls the Edit Customer Details tab on Debts and Customer Statements."],
          ["customer_merge_enabled", "Customer merging", "Controls merge tools in the Debt Desk and Customer Statement emergency review."],
      ...
      {loading ? <div style={{ marginTop: 12, color: "#64748b" }}>Loading the current store controls…</div> : null}
