...
export async function normalizeCustomerPortrait(file) {
  ...
  if (!best || !SAFE_PHOTO_PATTERN.test(best)) throw new Error("The photo could not be compressed safely.");
  return best;
}
...
export function CustomerPortrait({ customerId, src = "", name = "Customer", size = "medium", className = "", fallback = "initials" }) {
  ...
  return (
    <div className={`customer-portrait customer-portrait--${size} ${fallback === "blank" ? "is-blankable" : ""} ${className}`.trim()} aria-label={`${name} photo`}>
      {remoteSource ? <img src={remoteSource} alt={`${name} profile`} /> : fallback === "blank" ? <span className="customer-portrait__blank" aria-hidden="true" /> : <span>{initials(name)}</span>}
    </div>
  );
}
...
export function CustomerPortraitPicker({ value = "", name = "Customer", onChange, onError, compact = false }) {
  ...
  async function handleFile(event) {
    ...
    setProcessing(true);
    try {
      onChange?.(await normalizeCustomerPortrait(file));
      onError?.("");
    } catch (error) {
  ...
  return (
    <div className={`customer-photo-picker ${compact ? "is-compact" : ""}`}>
      ...
      <div className="customer-photo-picker__copy">
        ...
        <span>Any normal image is accepted. Chalin crops it to a neat passport portrait and compresses it automatically.</span>
