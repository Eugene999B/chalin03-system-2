...
function readAsImageElement(file) {
  return new Promise((resolve, reject) => {
    ...
    image.onerror = () => { URL.revokeObjectURL(url); reject(new Error("This image could not be read by the browser.")); };
    image.src = url;
  });
}
...
export function CustomerPortrait({ customerId, src = "", name = "Customer", size = "medium", className = "" }) {
  ...
  return <div className={`customer-portrait customer-portrait--${size} ${className}`.trim()} aria-label={`${name} photo`}>
...
export function CustomerPortraitPicker({ value = "", name = "Customer", onChange, onError, compact = false }) {
  ...
  async function handleFile(event) {
    ...
    setProcessing(true);
    try { onChange?.(await normalizeCustomerPortrait(file)); onError?.(""); }
    catch (error) { onError?.(error?.message || "The customer photo could not be prepared. Choose another image."); }
    finally { setProcessing(false); }
  }
  return <div className={`customer-photo-picker ${compact ? "is-compact" : ""}`}>
    <div className="customer-photo-picker__portrait"><CustomerPortrait src={value} name={name} size={compact ? "small" : "large"} /></div>
    <div className="customer-photo-picker__copy"><strong>Customer photo <em>Optional</em></strong><span>Any normal image is accepted. Chalin crops it to a neat passport portrait and compresses it automatically.</span><div className="customer-photo-picker__actions"><label className="customer-photo-picker__button is-primary">{processing ? "Preparing…" : value ? "Change photo" : "Add photo"}<input type="file" accept="image/*" onChange={handleFile} disabled={processing} hidden /></label>{value ? <button type="button" className="customer-photo-picker__button" onClick={() => onChange?.("")} disabled={processing}>Remove</button> : null}</div></div>
  </div>;
}
