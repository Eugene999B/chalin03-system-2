...
function roleLabel(role) {
  return role === "admin" ? "Admins / Boss" : role === "manager" ? "Managers" : "Auditors";
}

function normaliseRoles(rule) {
  return String(rule?.target_role || "admin,manager,auditor")
    .split(",")
...
export default function ExecutiveBusinessIntelligenceSettingsPanel() {
  ...
  async function updateRule(ruleCode, patch, successText) {
    ...
    if (!current?.id) {
      setError("The executive intelligence rule is not available yet. Reload after the backend scheduler has initialized it.");
      return;
    }
    setSaving(`${ruleCode}:${Object.keys(patch).join(",")}`);
    setNotice("");
    setError("");
    try {
  ...
  function toggleRole(rule, role) {
    const currentRoles = new Set(normaliseRoles(rule));
    if (currentRoles.has(role)) currentRoles.delete(role);
    else currentRoles.add(role);
    const nextRoles = ROLES.filter((candidate) => currentRoles.has(candidate));
    if (!nextRoles.length) {
    ...
    void updateRule(
      rule.rule_code,
      { target_role: nextRoles.join(",") },
      `${rule.rule_name} recipients updated.`
    );
  }

  if (loading) {
    return <section style={{ margin: "18px 0", padding: 20, borderRadius: 20, border: "1px solid #dbe3ef", background: "#fff" }}>Loading executive intelligence settings…</section>;
  }
    ...
    <section style={{ margin: "18px 0", padding: 22, borderRadius: 22, border: "1px solid #dbe3ef", background: "linear-gradient(135deg,#fff,#f8fafc)", boxShadow: "0 18px 45px rgba(15,23,42,.06)" }}>
      <div style={{ marginBottom: 16 }}>
        ...
        <p style={{ margin: "7px 0 0", color: "#64748b", lineHeight: 1.5, fontWeight: 650 }}>
          Configure who receives the automatic management intelligence. The system produces one consolidated report per period and sends it after the configured business closing window.
        </p>
      </div>
      ...
      <div style={{ display: "grid", gap: 12 }}>
        {REPORTS.map((report) => {
          ...
          const roles = new Set(normaliseRoles(rule));
          ...
          return (
            <article key={report.code} style={{ padding: 16, border: "1px solid #e2e8f0", borderRadius: 18, background: "#fff" }}>
              ...
              <div style={{ marginTop: 14, display: "flex", flexWrap: "wrap", gap: 8 }}>
                {ROLES.map((role) => (
                  <label key={role} style={{ display: "inline-flex", alignItems: "center", gap: 7, padding: "8px 10px", borderRadius: 12, border: "1px solid #dbe3ef", background: roles.has(role) ? "#fff8dc" : "#f8fafc", color: "#334155", fontWeight: 800, cursor: "pointer" }}>
                    <input type="checkbox" checked={roles.has(role)} onChange={() => toggleRole(rule, role)} disabled={!rule?.id || Boolean(saving)} />
                    {roleLabel(role)}
                  </label>
                ))}
              </div>
              ...
              <div style={{ marginTop: 10, color: "#64748b", fontSize: 12, fontWeight: 700 }}>
                SMS delivery: {smsAllowed ? "Enabled" : "Disabled"}. Report timing remains governed by the server-side closing window so the scheduler cannot send duplicates.
              </div>
            </article>
          );
        })}
