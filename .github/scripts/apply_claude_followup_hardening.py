...
# 1. Remove the dormant password-only Owner Break-Glass login implementation.
...
# 2. Add one canonical correction-evidence presentation for all Daily Closing outputs.
...
closing = replace_once(
    ...
    helper_anchor,
    helper,
    "Daily Closing correction helper insertion",
)
...
closing = replace_once(
    ...
    old_expense_query,
    new_expense_query,
    "Daily Closing expense evidence query",
)
...
# 3. Add source-level regression tests that fail if either hardening regresses.
...
test("Daily Closing exposes correction evidence consistently in all outputs", () => {
  ...
  assert.match(
    source,
    /wordCell\(expense\.display_category \|\| expense\.category/
  );
});
""")


# 4. Record the post-release hardening without changing the original audit score.
release_path = Path("docs/RELEASE_2026-07-25_PHASE1_POST_PHASE1.md")
release = release_path.read_text()
appendix_heading = "## Independent post-release review follow-up"
if appendix_heading not in release:
    release += """
