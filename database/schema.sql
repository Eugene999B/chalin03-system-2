...
CREATE TABLE user_permission_overrides (
    ...
    INDEX idx_permission_override_user_workspace (user_id, workspace_code, revoked_at, expires_at),
    INDEX idx_permission_override_permission (permission_code, effect, revoked_at),
...
CREATE TABLE security_event_dismissals (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
...
-- CHALIN 03 RELEASE 3F-B INSTALLMENT TABLES
...
-- ============================================================
-- RELEASE 3F-F: WORKER HR LETTERS
-- ============================================================
CREATE TABLE IF NOT EXISTS worker_hr_letters (
    ...
    CONSTRAINT fk_worker_hr_letters_worker FOREIGN KEY (worker_id) REFERENCES worker_profiles(id) ON DELETE RESTRICT,
    CONSTRAINT fk_worker_hr_letters_issued_by FOREIGN KEY (issued_by) REFERENCES users(id) ON DELETE SET NULL,
    CONSTRAINT fk_worker_hr_letters_cancelled_by FOREIGN KEY (cancelled_by) REFERENCES users(id) ON DELETE SET NULL,
    CONSTRAINT fk_worker_hr_letters_created_by FOREIGN KEY (created_by) REFERENCES users(id) ON DELETE SET NULL,
    CONSTRAINT fk_worker_hr_letters_updated_by FOREIGN KEY (updated_by) REFERENCES users(id) ON DELETE SET NULL
);

INSERT INTO schema_migrations (migration_name, description)
VALUES ('20260719_worker_hr_letters', 'Adds worker-linked employment and HR correspondence records with PDF generation, finalization status and acknowledgement evidence.')
ON DUPLICATE KEY UPDATE description = VALUES(description);
