...
test("runtime cost controls do not apply outside production", () => {
  const env = {
    NODE_ENV: "test",
    EQUIPMENT_FINANCE_BOSS_ALERT_POLL_MS: "1",
    EQUIPMENT_FINANCE_PAYMENT_MESSAGE_INTERVAL_MS: "1",
    NOTIFICATION_SYNC_INTERVAL_MINUTES: "1",
    SMS_DELIVERY_POLL_INTERVAL_MS: "1",
  };

  const result = applyRuntimeCostControl(env);

  assert.equal(result.applied, false);
  assert.equal(env.EQUIPMENT_FINANCE_BOSS_ALERT_POLL_MS, "1");
  assert.equal(env.EQUIPMENT_FINANCE_PAYMENT_MESSAGE_INTERVAL_MS, "1");
});
