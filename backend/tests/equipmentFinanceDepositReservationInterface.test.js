...
const page = read("frontend", "src", "pages", "EquipmentFinanceDepositReservationPage.jsx");
...
test("deposit is limited to authorised Finance managers and accountants", () => {
  assert.match(page, /DEPOSIT_ROLES/);
...
test("partial deposits remain separate from full excavator reservation", () => {
  ...
  assert.match(page, /Confirm reservation of the exact excavator/);
