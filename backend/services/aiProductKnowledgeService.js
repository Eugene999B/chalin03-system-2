...
const LIVE_RECORD_REQUEST_PATTERN = /\b(?:show|list|find|lookup|pull|retrieve|fetch|check|investigate|analy[sz]e|review|open|tell me|give me)\b[\s\S]{0,100}\b(?:customer|worker|employee|staff|salary|payroll|payment|sale|sales|sold|selling|stock|transaction|invoice|receipt|debt|balance|inventory|account|application|contract|arrears|collection|expense|revenue|profit|cash|bank|supplier|store|branch)\b|\b(?:today'?s?|current|currently|latest|live|actual|real-time|right now|this month|this week|outstanding|overdue)\b[\s\S]{0,100}\b(?:sale|sales|sold|selling|payment|payments|stock|inventory|balance|debt|debts|arrears|collection|collections|payroll|salary|salaries|invoice|invoices|transaction|transactions|cash|revenue|profit|worker|employee|customer|performance|operations|health|store|branch)\b|\b(?:sale|sales|sold|selling|sell|revenue|cash|payments?|collections?|invoice|invoices|invoiced|billing)\b[\s\S]{0,100}\b(?:today|yesterday|this week|this month|current|latest|live|right now)\b|\b(?:production|produce|produced|dispatch|dispatched|fuel|diesel|stockpile|utili[sz]ation|breakdown|operating cost|cost per unit|mining cost|incident|crew|site closing)\b[\s\S]{0,120}\b(?:today|yesterday|this week|this month|current|currently|latest|live|actual|real-time|right now)\b|\b(?:today'?s?|yesterday|this week|this month|current|currently|latest|live|actual|real-time|right now)\b[\s\S]{0,120}\b(?:production|produce|produced|dispatch|dispatched|fuel|diesel|stockpile|utili[sz]ation|breakdown|operating cost|cost per unit|mining cost|incident|crew|site closing)\b|\b(?:whole[- ]system|group|company[- ]wide|all (?:businesses|workspaces|operations))\b[\s\S]{0,80}\b(?:performance|operations|health|figures|numbers|results|position|status|today|current|latest|live)\b|\b(?:my|our|this|that)\s+(?:customer|worker|employee|staff|salary|payroll|payment|sale|sales|transaction|invoice|receipt|debt|balance|stock|inventory|account|application|contract|arrears|collection|expense|revenue|profit|cash|bank|supplier|store|branch)\b|\b(?:worker|employee|staff|customer)\s+(?:salary|wage|pay|debt|balance|account|payment|payments)\b|\b(?:salary|wage|pay)\s+(?:of|for)\s+(?:the\s+)?(?:worker|employee|staff|person)\b|\b(?:which|what)\s+(?:customer|worker|employee|staff)\b|\b(?:at|in|from|for)\s+(?:the\s+)?(?:main|head|first|second|current|selected|same|other)?\s*(?:store|branch|site|location)\b|\b(?:main|head)\s+(?:store|branch)\b/i;

const FINANCE_LIVE_RECORD_PATTERN = /(?:\b(?:today'?s?|current|currently|latest|live|actual|real-time|right now|this month|this week|outstanding|overdue)\b[\s\S]{0,140}\b(?:equipment finance|installment finance|instalment finance|finance portfolio|portfolio value|portfolio balance|finance balance|finance arrears|installment arrears|instalment arrears|finance collections?|finance payments?|credit applications?|approved applications?|finance reconciliation)\b)|(?:\b(?:equipment finance|installment finance|instalment finance|finance portfolio|portfolio value|portfolio balance|finance balance|finance arrears|installment arrears|instalment arrears|finance collections?|finance payments?|credit applications?|approved applications?|finance reconciliation)\b[\s\S]{0,140}\b(?:today|current|currently|latest|live|actual|real-time|right now|this month|this week|outstanding|overdue)\b)|(?:\b(?:show|list|find|lookup|pull|retrieve|fetch|check|investigate|analy[sz]e|review|tell me|give me)\b[\s\S]{0,100}\b(?:finance portfolio|equipment finance|installment finance|instalment finance|finance arrears|finance collections?|credit applications?|finance reconciliation)\b)/i;
...
function isLikelyLiveRecordRequest(value) {
  ...
  if (FINANCE_LIVE_RECORD_PATTERN.test(text)) return true;
