...
const LIVE_RECORD_REQUEST_PATTERN = /\b(?:show|list|find|lookup|pull|retrieve|fetch|check|investigate|analy[sz]e|review|open)\b[\s\S]{0,80}\b(?:customer|worker|employee|staff|salary|payroll|payment|sale|sales|transaction|invoice|receipt|debt|balance|stock|inventory|account|application|contract|arrears|collection|expense|revenue|profit|cash|bank|supplier)\b|\b(?:today'?s?|current|currently|latest|live|actual|real-time|right now|this month|this week|outstanding|overdue)\b[\s\S]{0,80}\b(?:sale|sales|payment|payments|stock|inventory|balance|debt|debts|arrears|collection|collections|payroll|salary|salaries|invoice|invoices|transaction|transactions|cash|revenue|profit|worker|employee|customer|performance|operations|health)\b|\b(?:whole[- ]system|group|company[- ]wide|all (?:businesses|workspaces|operations))\b[\s\S]{0,80}\b(?:performance|operations|health|figures|numbers|results|position|status|today|current|latest|live)\b|\b(?:my|our|this|that)\s+(?:customer|worker|employee|staff|salary|payroll|payment|sale|sales|transaction|invoice|receipt|debt|balance|stock|inventory|account|application|contract|arrears|collection|expense|revenue|profit|cash|bank|supplier)\b|\b(?:worker|employee|staff|customer)\s+(?:salary|wage|pay|debt|balance|account|payment|payments)\b|\b(?:salary|wage|pay)\s+(?:of|for)\s+(?:the\s+)?(?:worker|employee|staff|person)\b|\b(?:which|what)\s+(?:customer|worker|employee|staff)\b|\bhow\s+much\s+(?:does|did|is|are|was|were)\b/i;
