...
async function deliverExecutivePackSms(request) {
  try {
    ...
    const result = await sendSmsAlertToPhone({ branchId: Number(request.user?.branch_id || 1), phone, message, smsType: "executive_intelligence_pack", sentBy: request.user?.id || null, sourceReference: smsSourceReference });
    console.info("Executive intelligence SMS delivery attempt recorded.", { recipientId, smsStatus: result?.status || "unknown", logId: result?.log_id || null });
  } catch (error) {
