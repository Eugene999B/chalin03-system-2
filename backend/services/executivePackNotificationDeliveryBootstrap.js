...
function buildTwoMessageSms({ intelligence, role }) {
  ...
  if (role === "auditor") {
    analysis = `CHALIN03 AUDITOR ${range}: ${spare.sales_count || 0} completed sales worth ${compactMoney(spare.revenue)}; ${percent(collection)} collected, leaving ${compactMoney(gap)} open. Expenses ${compactMoney(spare.expenses)} = ${percent(expenseRatio)} of sales; ${spare.voided_sales_count || 0} voids worth ${compactMoney(spare.voided_sales_value)}; Finance ${compactMoney(finance.overdue_amount)} overdue, ${finance.overdue_accounts || 0} accounts.`;
