...
async function getCustomerFeatureControls(branchId) {
  ...
  await pool.query(
    ...
    [normalizedBranchId, DEFAULT_CONTROLS.customer_identity_editing_enabled ? 1 : 0, DEFAULT_CONTROLS.customer_merge_enabled ? 1 : 0]
  );
  const [rows] = await pool.query(
    `SELECT branch_id, customer_identity_editing_enabled, customer_merge_enabled, created_at, updated_at
       FROM customer_feature_controls WHERE branch_id = ? LIMIT 1`,
    [normalizedBranchId]
  );
  const row = rows[0] || {};
  return {
    branch_id: normalizedBranchId,
    customer_identity_editing_enabled: normalizeFlag(row.customer_identity_editing_enabled, DEFAULT_CONTROLS.customer_identity_editing_enabled),
    customer_merge_enabled: normalizeFlag(row.customer_merge_enabled, DEFAULT_CONTROLS.customer_merge_enabled),
    created_at: row.created_at || null,
    updated_at: row.updated_at || null,
  };
}

async function updateCustomerFeatureControls(branchId, { customer_identity_editing_enabled, customer_merge_enabled } = {}) {
  const normalizedBranchId = normalizeBranchId(branchId);
  const current = await getCustomerFeatureControls(normalizedBranchId);
  const next = {
    customer_identity_editing_enabled: normalizeFlag(customer_identity_editing_enabled, current.customer_identity_editing_enabled),
    customer_merge_enabled: normalizeFlag(customer_merge_enabled, current.customer_merge_enabled),
  };
  await pool.query(
    ...
    [normalizedBranchId, next.customer_identity_editing_enabled ? 1 : 0, next.customer_merge_enabled ? 1 : 0]
  );
  return getCustomerFeatureControls(normalizedBranchId);
}

function getFeatureDisabledMessage(featureName) {
  if (featureName === "customer_identity_editing") return "Customer identity editing is disabled by an administrator for this store.";
  if (featureName === "customer_merge") return "Customer merging is disabled by an administrator for this store.";
