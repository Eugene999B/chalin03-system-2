...
async function syncLegacyOwnership(db) {
  ...
  for (const row of rows) {
    ...
    const entityType = type.includes("customer") ? "customer" : type.includes("fleet") || type.includes("asset") || type.includes("equipment") || type.includes("excavator") ? "fleet_asset" : null;
...
async function ownedIds(db, type) {
  ...
  const [rows] = await db.query(`SELECT entity_id FROM ${quote(OWNERSHIP_TABLE)} WHERE workspace_code=? AND entity_type=?`, [WORKSPACE, type]);
  return uniq(rows.map((row) => row.entity_id));
}

function harvest(row, scope) {
...
async function scanAgreements(db, predicate) {
  ...
  for (const row of rows) {
    ...
    if (!predicate || predicate(row)) { harvest(row, scope); if (row.id != null) scope.agreements.push(row.id); }
  }
  return scope;
}

function mergeScopes(...scopes) {
  const result = { customers: [], assets: [], applications: [], agreements: [], payments: [], quotations: [] };
  for (const scope of scopes) for (const key of Object.keys(result)) result[key].push(...(scope[key] || []));
  return Object.fromEntries(Object.entries(result).map(([key, values]) => [key, uniq(values)]));
}

async function collectScope(db) {
  const ownership = { customers: await ownedIds(db, "customer"), assets: await ownedIds(db, "fleet_asset"), applications: [], agreements: [], payments: [], quotations: [] };
  const scanned = await scanInstallmentTables(db);
  const agreements = await scanAgreements(db);
  return mergeScopes(ownership, scanned, agreements);
}

async function collectEntityScope(db, entityType, entityId) {
  const id = Number(entityId);
  const ownershipType = entityType === "asset" ? "fleet_asset" : "customer";
  const owned = (await ownedIds(db, ownershipType)).includes(id);
  const direct = await scanInstallmentTables(db, (row) => {
    const columns = entityType === "asset" ? ASSET_COLUMNS : CUSTOMER_COLUMNS;
    return columns.some((column) => row[column] != null && Number(row[column]) === id);
  });
  const directAgreements = await scanAgreements(db, (row) => {
    const columns = entityType === "asset" ? ASSET_COLUMNS : CUSTOMER_COLUMNS;
    return columns.some((column) => row[column] != null && Number(row[column]) === id);
  });
  const linkedIds = mergeScopes(direct, directAgreements);
  linkedIds.customers = entityType === "customer" ? [id, ...linkedIds.customers] : linkedIds.customers;
  linkedIds.assets = entityType === "asset" ? [id, ...linkedIds.assets] : linkedIds.assets;
  return { owned, scope: linkedIds };
}
...
async function collectImpact(db) {
  ...
  for (const [table, cols, ids] of [
    ["equipment_sale_agreements", ["id"], scope.agreements],
    ["equipment_credit_applications", ["id"], scope.applications],
    ["equipment_sales_quotations", ["id"], scope.quotations],
  ]) {
    const count = await countRelation(db, table, cols, ids);
    if (count) impact.push({ table, rows: count });
  }
...
async function getEntityImpact(db, entityType, entityId) {
  const ownershipType = entityType === "asset" ? "fleet_asset" : "customer";
  const { owned, scope } = await collectEntityScope(db, entityType, entityId);
  ...
  const protectedRefs = refs.filter(protectedReference);
  return {
    ...
    preservation_reason: protectedRefs.length ? "Referenced by another business module." : refs.length ? "Still referenced by another database record." : null,
  };
}

async function deleteRelation(db, table, candidateColumns, ids, deleted) {
...
async function deleteScopedRows(db, scope, deleted) {
  ...
  for (const table of [...INSTALLMENT_TABLES].reverse()) {
    await deleteRelation(db, table, AGREEMENT_COLUMNS, scope.agreements, deleted);
    await deleteRelation(db, table, APPLICATION_COLUMNS, scope.applications, deleted);
    await deleteRelation(db, table, PAYMENT_COLUMNS, scope.payments, deleted);
    await deleteRelation(db, table, CUSTOMER_COLUMNS, scope.customers, deleted);
    await deleteRelation(db, table, ASSET_COLUMNS, scope.assets, deleted);
  }
...
async function removeOwnedMaster(db, entityType, entityId, deleted) {
  const impact = await getEntityImpact(db, entityType, entityId);
  if (!impact.master_delete_eligible) return { entity_type: entityType, entity_id: entityId, deleted: false, reason: impact.preservation_reason || "remaining_reference", impact };
...
async function getEntityImpact(entityType, entityId) {
  ...
  try { return await getEntityImpact(db, entityType, Number(entityId)); } finally { db.release(); }
}
...
async function deleteEntityTransaction(entityType, entityId) {
  ...
  try {
    ...
    const impact = await getEntityImpact(db, entityType, id);
    ...
    const deleted = [];
    await deleteScopedRows(db, impact.installment_scope, deleted);
...
async function resetInstallmentTransaction() {
  ...
  try {
    ...
    await db.commit();
    return { status: "success", scope, impact, deleted, masters, message: "Installment Finance was cleared by the unified deletion engine." };
  } catch (error) {
    try { await db.rollback(); } catch (_) {}
    throw error;
  } finally { db.release(); }
