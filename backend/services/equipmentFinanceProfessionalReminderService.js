...
function classifyReminder(row, settings, today) {
  ...
  if (daysUntil < 0) {
    ...
    if (daysPast === 1 || daysPast % repeat === 0) {
      return {
        type: "overdue",
        days: daysPast,
...
async function reminderCandidates({ today = ghanaDate(), limit = 500 } = {}) {
  ...
  for (const row of rows) {
    const classification = classifyReminder(row, settings, today);
...
async function runProfessionalReminderSync({
  ...
  if (!bypassTime && !insideAllowedTime(settings)) {
    return { sent: 0, failed: 0, skipped: reminders.length, reason: "outside_reminder_hour" };
  }
  if (!bypassTime && weekendBlocked(settings)) {
  ...
  for (const reminder of reminders.slice(0, 100)) {
    ...
    const [claim] = await pool.query(
      `INSERT IGNORE INTO equipment_sales_reminder_log (
         ...
         reminder_type, recipient_phone, delivery_status, message_preview,
         sent_by
       ) VALUES (?, ?, ?, ?, ?, ?, 'pending', ?, ?)`,
      [
        reminder.hire_location_id,
        reminder.agreement_id,
    ...
    await pool.query(
      `UPDATE equipment_sales_reminder_log
       SET sms_log_id = ?, delivery_status = ?, sent_at = NOW()
       WHERE reminder_key = ?`,
      [sms.log_id || null, status, reminder.reminder_key]
    );
