...
function appendIdentityAnnex(doc, context) {
  ...
  doc.image(context.photo.buffer, left + 8, bodyY + 8, { fit: [photoWidth - 16, photoHeight - 16], align: "center", valign: "center" });
  ...
  doc.fillColor("#174f35").font("Helvetica-Bold").fontSize(10).text("Protected customer identity evidence", left + 14, evidenceY + 14, { width: width - 28 });
  doc.fillColor("#42564a").font("Helvetica").fontSize(8).text("This full-frame customer photograph is stored in the encrypted Equipment Installment Finance document vault. It is included to support customer identification and does not replace the official Ghana Card, passport or other identification evidence recorded in the KYC file.", left + 14, evidenceY + 33, { width: width - 28, lineGap: 3, align: "justify" });
  doc.fillColor("#6b766f").font("Helvetica").fontSize(6.5).text(`Photo evidence ${context.photo.documentNumber} | SHA-256 ${String(context.photo.checksum || "").slice(0, 24)}...`, left, evidenceY + 102, { width, align: "center" });
}
...
async function renderCompletionPdf(document, options = {}) {
  ...
  return annexContext.run({ document, photo, appended: false }, () => renderBasePdf(document, options));
}
