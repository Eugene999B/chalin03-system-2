...
const DEFAULTS = Object.freeze(Object.fromEntries(CATEGORIES.map((category) => [category, true])));
...
async function getNotificationPolicy(connection = pool) {
  ...
  const row = rows[0];
  const controls = {};
  for (const category of CATEGORIES) controls[category] = row ? Boolean(Number(row[category])) : DEFAULTS[category];
  return controls;
}
...
async function updateNotificationPolicy(input = {}, connection = pool) {
  ...
  for (const category of CATEGORIES) {
    ...
    const parsed = parseBoolean(input[category]);
    if (parsed === null) {
      const error = new Error(`${LABELS[category]} setting must be true or false.`);
      error.statusCode = 400;
      error.code = "INVALID_NOTIFICATION_CONTROL";
      throw error;
    }
    next[category] = parsed;
    if (next[category] !== current[category]) changed.push(category);
  }
...
async function isNotificationEnabled(category, connection = pool) {
  ...
  try {
    ...
    console.warn(`Installment notification policy check failed-open for ${key}:`, error.message);
    return true;
  }
}
...
module.exports = {
  ...
  DEFAULTS,
  LABELS,
  ensureNotificationPolicySchema,
