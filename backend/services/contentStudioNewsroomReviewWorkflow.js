...
const { configFor } = require("./contentStudioNewsroomSchema");
const {
  clampLimit,
  getEntityDetails,
  loadEntityForUpdate,
...
async function submitEntityVersion({
  ...
  try {
    ...
    if (version.version_status !== "draft") {
      throw new ContentStudioError("Only a draft Newsroom version may be submitted.", {
        code: "NEWSROOM_VERSION_NOT_DRAFT",
        statusCode: 409,
      });
    }
    ...
    if (pendingRows[0]) {
      throw new ContentStudioError(
        "This Newsroom version already has a pending review.",
        { code: "NEWSROOM_REVIEW_PENDING", statusCode: 409 }
      );
    }
...
async function decideEntityApproval({ kind, approvalId, decision, note, user, req }) {
  ...
  if (!id || !["approved", "rejected"].includes(normalizedDecision)) {
    throw new ContentStudioError("Choose Approve or Reject.", {
  ...
  try {
    ...
    if (!approval || approval.entity_type !== entityType) {
      throw new ContentStudioError("Newsroom approval request not found.", {
    ...
    if (Number(approval.requested_by) === Number(user?.id)) {
      throw new ContentStudioError(
        "The Newsroom submitter cannot approve the same version.",
...
async function listNewsroomApprovals({ kind, assignedTo, limit, offset } = {}) {
  ...
  if (kind) {
    ...
    values.push(configFor(kind).entityType);
  } else {
    filters.push("a.entity_type IN ('news_article','announcement')");
  }
