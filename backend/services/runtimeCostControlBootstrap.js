const MINIMUMS = Object.freeze({
  // Keep important executive alerts responsive while eliminating rapid database polling.
  EQUIPMENT_FINANCE_BOSS_ALERT_POLL_MS: 5 * 60 * 1000,
  // Payment receipts are sent synchronously; this loop is retry/remediation work.
  EQUIPMENT_FINANCE_PAYMENT_MESSAGE_INTERVAL_MS: 15 * 60 * 1000,
  // Notification reconciliation is operational maintenance, not a real-time channel.
  NOTIFICATION_SYNC_INTERVAL_MINUTES: 60,
  // Delivery webhooks remain available; polling is fallback status reconciliation.
...
function applyRuntimeCostControl(env = process.env) {
  if (!isProduction(env)) {
    return { applied: false, production: false };
  }
