...
function sanitizeSource(input = {}, fallback = {}) {
  ...
  const sourceType = clean(input.source_type ?? fallback.source_type ?? "other", 40);
  const visibility = clean(input.visibility ?? fallback.visibility ?? "workspace", 40);
  const title = clean(input.title ?? fallback.title, 255);
  const effectiveFrom = normalizeDate(input.effective_from ?? fallback.effective_from);
  const expiresAt = normalizeDate(input.expires_at ?? fallback.expires_at);

  if (!sourceKey || !SOURCE_TYPES.includes(sourceType) || !VISIBILITIES.includes(visibility) || !title) {
  ...
  return Object.freeze({
    ...
    owner_workspace_code:
      visibility === "public"
        ? clean(input.owner_workspace_code ?? fallback.owner_workspace_code, 50)
        : clean(input.owner_workspace_code ?? fallback.owner_workspace_code, 50),
    visibility,
    title,
...
async function loadVersion(connection, sourceId, versionId, forUpdate = false) {
  const [rows] = await connection.query(
    `SELECT * FROM ai_knowledge_versions
     WHERE id = ? AND source_id = ? LIMIT 1${forUpdate ? " FOR UPDATE" : ""}`,
...
async function createKnowledgeSourceDraft({ input, user, req, connection = null } = {}) {
  ...
  try {
    ...
    if (error?.code === "ER_DUP_ENTRY") {
      throw new AiKnowledgeError("A knowledge source with this key already exists.", {
        code: "AI_KNOWLEDGE_SOURCE_DUPLICATE",
        statusCode: 409,
      });
    }
...
async function createKnowledgeVersion({ sourceId, input, user, req } = {}) {
  ...
  try {
    ...
    if (source.source_status === "archived") {
      throw new AiKnowledgeError("Archived knowledge cannot receive a new version.", {
    ...
    const [latestRows] = await connection.query(
      `SELECT * FROM ai_knowledge_versions
       WHERE source_id = ? ORDER BY version_number DESC, id DESC LIMIT 1 FOR UPDATE`,
      [source.id]
    );
    ...
    await connection.query(
      "UPDATE ai_knowledge_sources SET updated_by = ?, updated_at = UTC_TIMESTAMP() WHERE id = ?",
    ...
    await writeAuditEvent({
      ...
      entityType: "ai_knowledge_source",
      entityId: source.id,
      metadata: { version_id: Number(result.insertId), version_number: nextNumber },
    });
...
async function updateKnowledgeDraft({ sourceId, versionId, input, user, req } = {}) {
  ...
  try {
    ...
    if (existing.version_status !== "draft") {
      throw new AiKnowledgeError("Only a draft knowledge version may be edited.", {
        code: "AI_KNOWLEDGE_VERSION_NOT_EDITABLE",
        statusCode: 409,
      });
    }
    const version = sanitizeVersion(input, {
    ...
    await connection.query(
      `UPDATE ai_knowledge_sources
       SET title = ?, description = COALESCE(?, description),
           ...
           owner_workspace_code = COALESCE(?, owner_workspace_code),
           visibility = COALESCE(?, visibility), updated_by = ?,
           updated_at = UTC_TIMESTAMP()
       WHERE id = ?`,
      [
        clean(input.title, 255) || source.title,
        input.description === undefined ? null : clean(input.description, 1000),
        input.source_reference === undefined
          ? null
          : clean(input.source_reference, 500),
        input.owner_workspace_code === undefined
          ? null
          : clean(input.owner_workspace_code, 50),
        input.visibility === undefined ? null : clean(input.visibility, 40),
        user?.id || null,
        source.id,
      ]
    );
    await writeAuditEvent({
      ...
      entityType: "ai_knowledge_source",
      entityId: source.id,
      metadata: { version_id: existing.id, checksum_sha256: version.checksum_sha256 },
    });
    await connection.commit();
    return true;
  } catch (error) {
...
async function submitKnowledgeVersion({ sourceId, versionId, assignedTo, note, user, req } = {}) {
  const connection = await pool.getConnection();
  try {
    await connection.beginTransaction();
    const source = await loadSource(connection, sourceId, true);
    ...
    if (version.version_status !== "draft") {
      throw new AiKnowledgeError("Only a draft knowledge version may be submitted.", {
        code: "AI_KNOWLEDGE_VERSION_NOT_DRAFT",
        statusCode: 409,
      });
    }
    ...
    if (pending[0]) {
      throw new AiKnowledgeError("This knowledge version already awaits review.", {
        code: "AI_KNOWLEDGE_REVIEW_ALREADY_PENDING",
        statusCode: 409,
      });
    }
    const [result] = await connection.query(
      `INSERT INTO ai_knowledge_approvals (
         source_id, version_id, approval_status, requested_by, assigned_to, request_note
       ) VALUES (?, ?, 'pending', ?, ?, ?)`,
      [
        source.id,
        version.id,
    ...
    await connection.query(
      "UPDATE ai_knowledge_versions SET version_status = 'in_review' WHERE id = ?",
      [version.id]
    );
    await writeAuditEvent({
      connection,
...
async function decideKnowledgeApproval({ approvalId, decision, note, user, req } = {}) {
  const normalizedDecision = clean(decision, 20)?.toLowerCase();
  if (!positiveInteger(approvalId) || !["approved", "rejected"].includes(normalizedDecision)) {
  ...
  try {
    await connection.beginTransaction();
    const [rows] = await connection.query(
      "SELECT * FROM ai_knowledge_approvals WHERE id = ? LIMIT 1 FOR UPDATE",
      [positiveInteger(approvalId)]
    );
    const approval = rows[0];
    if (!approval || approval.approval_status !== "pending") {
      throw new AiKnowledgeError("Knowledge approval is unavailable or already decided.", {
        code: "AI_KNOWLEDGE_APPROVAL_NOT_PENDING",
        statusCode: 409,
      });
    }
    if (Number(approval.requested_by) === Number(user?.id)) {
      throw new AiKnowledgeError("The submitter cannot approve their own knowledge version.", {
        code: "AI_KNOWLEDGE_SELF_APPROVAL_BLOCKED",
        statusCode: 409,
      });
    }
    ...
    if (version.version_status !== "in_review") {
      throw new AiKnowledgeError("The exact knowledge version is no longer in review.", {
        code: "AI_KNOWLEDGE_APPROVAL_STATE_MISMATCH",
        statusCode: 409,
      });
    }
    await connection.query(
    ...
    await connection.query(
      "UPDATE ai_knowledge_versions SET version_status = ? WHERE id = ?",
...
async function publishKnowledgeVersion({ sourceId, versionId, user, req } = {}) {
  ...
  try {
    ...
    if (version.version_status !== "approved") {
      throw new AiKnowledgeError("Only an approved knowledge version may be published.", {
        code: "AI_KNOWLEDGE_VERSION_NOT_APPROVED",
        statusCode: 409,
      });
    }
    const [approvals] = await connection.query(
      `SELECT * FROM ai_knowledge_approvals
       WHERE source_id = ? AND version_id = ? AND approval_status = 'approved'
       ORDER BY decided_at DESC, id DESC LIMIT 1 FOR UPDATE`,
      [source.id, version.id]
    );
    const approval = approvals[0];
    if (!approval) {
      throw new AiKnowledgeError("Publishing requires an approved review record.", {
        code: "AI_KNOWLEDGE_APPROVED_REVIEW_REQUIRED",
        statusCode: 409,
      });
    }
    if (Number(approval.requested_by) === Number(user?.id)) {
      // Publishing is a separate permission; this check prevents a submitter
      // from using a compromised publisher path to complete their own version.
      throw new AiKnowledgeError("The original submitter cannot publish this version.", {
        code: "AI_KNOWLEDGE_SUBMITTER_PUBLISH_BLOCKED",
        statusCode: 409,
      });
    }
    await connection.query(
    ...
    await connection.query(
      "UPDATE ai_knowledge_approvals SET executed_at = UTC_TIMESTAMP() WHERE id = ?",
      [approval.id]
    );
    await writeAuditEvent({
      connection,
...
async function listKnowledgeSources({
  ...
  if (search) {
    filters.push("(s.title LIKE ? OR s.description LIKE ? OR s.source_key LIKE ?)");
...
async function getKnowledgeSourceDetails(sourceId) {
  try {
    ...
    const [versions] = await pool.query(
      `SELECT id, version_number, version_status, title, checksum_sha256,
              effective_from, expires_at, created_by, published_by,
...
function retrievalVisibility(persona, workspaceCode) {
  ...
  if (normalizedPersona === "executive") {
    return {
      sql: "s.visibility IN ('public', 'workspace', 'executive', 'restricted')",
      params: [],
    };
  }
  return {
  ...
  try {
    const [rows] = await pool.query(
      `SELECT s.source_key, s.source_type, s.owner_workspace_code,
              ...
              v.checksum_sha256, v.effective_from, v.expires_at, v.published_at
       FROM ai_knowledge_sources s
       ...
       WHERE s.source_status = 'active'
         ...
         AND (v.title LIKE ? ESCAPE '\\\\' OR v.body_text LIKE ? ESCAPE '\\\\')
       ORDER BY
         CASE WHEN v.title LIKE ? ESCAPE '\\\\' THEN 0 ELSE 1 END,
         v.published_at DESC, v.id DESC
       LIMIT ?`,
      [...visibility.params, like, like, like, safeLimit]
    );
