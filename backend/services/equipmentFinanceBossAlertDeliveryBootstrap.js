...
function money(value) { const number = Number(value); if (!Number.isFinite(number)) return ""; return `GHS ${number.toLocaleString("en-GH", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`; }
function parseMetadata(value) { if (!value) return {}; if (typeof value === "object") return value; try { const parsed = JSON.parse(String(value)); return parsed && typeof parsed === "object" ? parsed : {}; } catch { return {}; } }
function hasAmountSignal(metadata = {}, details = "") { const keys = Object.keys(metadata).map((key) => key.toLowerCase()); const text = `${keys.join(" ")} ${String(details || "").toLowerCase()}`; return /(amount|price|deposit|financed|balance|installment|payment|total|selling|hire_rate|periodic)/i.test(text); }
...
function isFinanceActivity(row) {
  ...
  if (entityType.includes("equipment") || entityType === "fleet_asset" || entityType === "equipment_sale_payment" || entityType === "equipment_sale_agreement") return /create|created|register|registered|update|updated|edit|edited|payment|deposit|agreement|reserve|reservation/i.test(`${action} ${actionType}`);
...
function findValue(metadata, keys) { for (const key of keys) if (metadata[key] !== undefined && metadata[key] !== null && metadata[key] !== "") return metadata[key]; return null; }

function buildActivityMessage(row) {
  const metadata = parseMetadata(row?.metadata_json), kind = eventKind(row, metadata), details = clean(row?.details, 420), entityId = clean(row?.entity_id, 80);
  ...
  switch (kind) {
    case "machine_created": return ["CHALIN 03 — Installment Finance Alert", `New excavator/equipment registered: ${subject}.${customer ? ` Customer: ${customer}.` : ""}${amountText}`, details || null, actor || null].filter(Boolean).join(" ");
    case "customer_created": return ["CHALIN 03 — Installment Finance Alert", `New finance customer created: ${customer || subject}.`, agreement ? `Agreement: ${agreement}.` : null, asset ? `Equipment: ${asset}.` : null, actor || null].filter(Boolean).join(" ");
    case "deposit": return ["CHALIN 03 — Installment Finance Alert", `Deposit recorded for ${subject}.${customer ? ` Customer: ${customer}.` : ""}`, amountText || null, method ? `Method: ${method}.` : null, receipt ? `Receipt: ${receipt}.` : null, actor || null].filter(Boolean).join(" ");
    case "payment": return ["CHALIN 03 — Installment Finance Alert", `Payment recorded for ${subject}.${customer ? ` Customer: ${customer}.` : ""}`, amountText || null, method ? `Method: ${method}.` : null, receipt ? `Receipt: ${receipt}.` : null, actor || null].filter(Boolean).join(" ");
    case "agreement": return ["CHALIN 03 — Installment Finance Alert", `Installment agreement activated: ${subject}.${customer ? ` Customer: ${customer}.` : ""}`, asset ? `Equipment: ${asset}.` : null, amountText || null, actor || null].filter(Boolean).join(" ");
    case "edited": return ["CHALIN 03 — Installment Finance Alert", `Finance record edited: ${subject}.`, fieldText ? `Changed: ${fieldText}.` : null, hasAmountSignal(metadata, details) ? "Amount/financial field was involved." : null, details && !fieldText ? details : null, actor || null].filter(Boolean).join(" ");
    default: return ["CHALIN 03 — Installment Finance Alert", details || `Finance activity recorded for ${subject}.`, actor || null].filter(Boolean).join(" ");
  }
}

async function professionalBossSettings() { const settings = await getProfessionalSettings(); return { enabled: Boolean(Number(settings?.boss_payment_alert_enabled)), phone: clean(settings?.boss_payment_alert_phone, 40) }; }
async function sourceAlreadyLogged(sourceReference) { try { const [rows] = await pool.query("SELECT id FROM sms_log WHERE source_reference = ? LIMIT 1", [sourceReference]); return rows.length > 0; } catch (error) { console.warn("Boss SMS duplicate check skipped:", error.message); return false; } }
...
async function deliverFinanceActivityBossAlert(row) {
  ...
  if (!settings.enabled || !settings.phone) { console.warn(`Finance boss alert skipped for activity ${row.id}: enabled=${settings.enabled} phone_configured=${Boolean(settings.phone)}.`); return; }
  const result = await sendSmsAlertToPhone({ branchId: Number(row.branch_id || 1), phone: settings.phone, message: buildActivityMessage(row), logMessage: `Boss alert: ${clean(row.action || row.entity_type || "Finance activity", 360)}.`, smsType: "equipment_finance_boss_alert", sentBy: row.user_id || null, sourceReference });
...
async function initialiseActivityCursor() {
  ...
  } catch (error) { console.warn("Finance boss alert cursor initialization skipped:", error.message); activityCursor = 0; }
}
  ...
  pollPromise = (async () => {
    try {
      const [rows] = await pool.query(
        `SELECT id, branch_id, user_id, action, details, workspace_code, entity_type, entity_id, action_type, outcome, severity, metadata_json
         FROM activity_log
         WHERE id > ? AND outcome = 'success' AND (
...
async function deliverEquipmentCreatedBossAlert(request) {
  const settings = await professionalBossSettings(); if (!settings.enabled || !settings.phone) return;
  const body = request?.body || {}, assetCode = clean(body.asset_code, 50).toUpperCase(), assetName = clean(body.asset_name, 150), identity = [assetCode, assetName].filter(Boolean).join(" — ") || "New equipment";
  await sendSmsAlertToPhone({ branchId: Number(request?.user?.branch_id || 1), phone: settings.phone, message: `CHALIN 03 — Installment Finance Alert New excavator/equipment registered: ${identity}.`, logMessage: `Boss alert: new equipment ${identity}.`, smsType: "equipment_finance_boss_alert", sentBy: request?.user?.id || null, sourceReference: `equipment-created:${assetCode || assetName || Date.now()}` });
}
...
module.exports = { buildActivityMessage, deliverEquipmentCreatedBossAlert, deliverFinanceActivityBossAlert, eventKind, installEquipmentFinanceBossAlertDelivery, isFinanceActivity, parseMetadata };
