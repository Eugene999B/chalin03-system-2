...
async function createEntityVersion({
  ...
  try {
    ...
    const gallery = kind === "project" ? await loadGallery(connection, id) : [];
    const latest = await loadLatestVersion(connection, kind, id, true);
    const current = snapshotFromRow(kind, entity, gallery);
    const base = latest
      ? parseJson(latest.snapshot_json, current)
      : current;
    ...
    await platformAudit(
      ...
      req,
      kind,
      `PUBLIC_${entityType.toUpperCase()}_VERSION_CREATED`,
      id,
  ...
  try {
    ...
    if (version.version_status !== "draft") {
      throw new ContentStudioError("Only a draft version may be edited.", {
        code: "PORTFOLIO_VERSION_NOT_EDITABLE",
        statusCode: 409,
      });
    }
    ...
    await connection.query(
      `UPDATE public_content_versions
       SET snapshot_json = ?, change_summary = ?
       WHERE id = ?`,
      [
    ...
    await platformAudit(
      ...
      req,
      kind,
      `PUBLIC_${entityType.toUpperCase()}_DRAFT_UPDATED`,
      id,
      { version_id: draftId }
    );
    await connection.commit();
