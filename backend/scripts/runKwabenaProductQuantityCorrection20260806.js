...
const PRODUCT_CORRECTIONS = Object.freeze([
  Object.freeze({
    product_id: 313,
    requested_name: "Bearing case",
    exported_name: "Bearing Case",
    exported_size: "None",
    exported_quantity: 8,
    quantity: 4,
  }),
  Object.freeze({
    ...
    exported_size: "None",
    exported_quantity: 49,
    quantity: 13,
    matching_note:
      "Matched to active product ID 263; inactive duplicate product ID 252 was excluded.",
  }),
  Object.freeze({
    product_id: 205,
    requested_name: "Track Shoe bolt",
    exported_name: "Track Shoe Bolt",
    exported_size: "All",
    exported_quantity: 466,
    quantity: 152,
  }),
  Object.freeze({
    product_id: 96,
    requested_name: "Binding wire",
    exported_name: "Binding Wire",
  ...
  Object.freeze({
    product_id: 218,
    requested_name: "Grease gun mouth",
    exported_name: "Grease Gun Mouth",
    exported_size: "All",
    exported_quantity: 32,
    quantity: 27,
  }),
  Object.freeze({
    product_id: 200,
  ...
  Object.freeze({
    product_id: 32,
    requested_name: "Sany Air cleaner",
    exported_name: "Air Cleaner",
    exported_size: "Sany",
    exported_quantity: 6,
    quantity: 0,
    matching_note:
      "Matched the shorthand to the active Air Cleaner row whose exact size is Sany.",
  }),
  Object.freeze({
    product_id: 46,
...
function validateCorrectionDefinitions(corrections = PRODUCT_CORRECTIONS) {
  if (corrections.length !== EXPECTED_CORRECTION_COUNT) {
    throw new Error(
      `Expected exactly ${EXPECTED_CORRECTION_COUNT} product corrections, received ${corrections.length}.`
    );
  }
...
function resolveExactCorrections(products, corrections = PRODUCT_CORRECTIONS) {
  ...
  if (missingIds.length) {
    throw new Error(
      `Exported production product IDs are missing: ${missingIds.join(
        ", "
      )}. No quantity was changed.`
    );
  }
...
async function applyResolvedCorrections(connection, resolved) {
  ...
  for (let index = 0; index < resolved.length; index += 1) {
    ...
    const reference = `KWABENA-COUNT-20260806-${String(index + 1).padStart(
      2,
      "0"
    )}`;
    const reason =
      "Boss-approved Main Store physical count received from Kwabena on 2026-08-05 and 2026-08-06.";
    ...
    if (Number(update?.affectedRows || 0) !== 1) {
      throw new Error(
        `Product ${item.product_id} was not updated exactly once. No correction was committed.`
      );
    }
  ...
  await connection.query(
    `INSERT INTO activity_log (branch_id, user_id, action, details)
     VALUES (?, NULL, 'KWABENA_MAIN_STORE_QUANTITY_CORRECTION', ?)`,
...
async function runKwabenaProductQuantityCorrection20260806() {
  ...
  try {
    ...
    const [products] = await connection.query(
      `SELECT id, branch_id, name, size, quantity, cost_price, selling_price,
              barcode, is_active
         FROM products
    ...
    if (result.applied.length !== PRODUCT_CORRECTIONS.length) {
      throw new Error(
        `Prepared ${result.applied.length} corrections instead of ${PRODUCT_CORRECTIONS.length}.`
      );
    }
    ...
    console.log(
      `Applied ${CORRECTION_RECORD} to ${result.applied.length} Main Store products on ${databaseName}.`
    );
