...
const MIGRATION_LOCK = "chalin03:equipment-finance:phase6-reporting-notifications";
const MIGRATION_RECORD = "equipment_finance_phase6_reporting_notifications";
const MIGRATION_FILE = "20260802_equipment_finance_phase6_reporting_notifications.sql";
const VERIFIER_FILE = "20260802_equipment_finance_phase6_reporting_notifications_verify.sql";
...
function readSql(filename) {
  ...
  if (!fs.existsSync(filePath)) {
    throw new Error(`Approved Phase 6 SQL file is missing: ${filePath}`);
  }
...
function validateVerifierResults(results) {
  if (results.length !== 5) {
    throw new Error(`Phase 6 verifier returned ${results.length} result sets instead of 5.`);
  }
  const [migrationRows, tables, columns, cutover, history] = results;
  if (migrationRows.length !== 1 || migrationRows[0].migration_name !== MIGRATION_RECORD) {
    throw new Error("The Phase 6 migration record was not verified.");
  }
  if (Number(tables[0]?.missing_phase6_tables || 0) !== 0) {
    throw new Error("One or more Phase 6 tables are missing.");
  }
  if (Number(columns[0]?.missing_phase6_message_columns || 0) !== 0) {
    throw new Error("The Phase 6 payment-message evidence columns are incomplete.");
  }
  if (Number(cutover[0]?.missing_cutover_state || 0) !== 0) {
    throw new Error("The Phase 6 customer SMS cutover state is missing.");
  }
  if (Number(cutover[0]?.invalid_phase6_sms_settings || 0) !== 0) {
    throw new Error("The Phase 6 boss alerts, customer receipts or automatic reminders are not enabled.");
  }
  if (Number(history[0]?.invalid_automatic_receipt_history || 0) !== 0) {
    throw new Error("The Phase 6 verifier found an automatic receipt before the production cutover.");
  }
}

async function runEquipmentFinancePhaseSixStartup() {
  ...
  try {
    ...
    if (!lockAcquired) throw new Error("Could not acquire the Phase 6 migration lock.");
    if (!(await migrationRecordExists(connection))) {
      await executeStatements(
        connection,
        splitSqlScript(readSql(MIGRATION_FILE)),
        "Equipment Finance Phase 6 migration"
      );
      console.log(`Applied ${MIGRATION_RECORD} on ${databaseName}.`);
    }
    const results = await executeStatements(
      connection,
      splitSqlScript(readSql(VERIFIER_FILE)),
      "Equipment Finance Phase 6 verifier"
    );
...
if (require.main === module) {
  runEquipmentFinancePhaseSixStartup().catch((error) => {
    console.error("Equipment Finance Phase 6 Railway startup gate failed.");
...
module.exports = {
  ...
  runEquipmentFinancePhaseSixStartup,
  splitSqlScript,
