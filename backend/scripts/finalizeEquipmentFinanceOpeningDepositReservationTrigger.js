...
function connectionOptions() {
  ...
  return {
    ...
    ssl: sslEnabled ? (encodedCa ? { ca: Buffer.from(encodedCa, "base64").toString("utf8"), rejectUnauthorized: true } : { rejectUnauthorized }) : undefined,
...
async function finalize() {
  ...
  try {
    ...
    if (!databaseName || !expected || databaseName !== expected) throw new Error(`Connected database ${databaseName || "(unknown)"} does not match expected production database ${expected || "(unset)"}.`);
    ...
    for (const statement of splitSqlScript(fs.readFileSync(sqlPath, "utf8"))) await connection.query(statement);

    const [[trigger]] = await connection.query(
      `SELECT TRIGGER_NAME, ACTION_STATEMENT FROM information_schema.TRIGGERS WHERE TRIGGER_SCHEMA = DATABASE() AND TRIGGER_NAME = 'trg_equipment_finance_reservation_gate_before_insert'`
    );
    const action = String(trigger?.ACTION_STATEMENT || "");
    if (!/v_deposit_received\s*\+\s*0\.01\s*<\s*v_deposit_required/i.test(action)) throw new Error("The Opening Deposit reservation trigger did not reach the approved final definition.");
    ...
    if (lockAcquired) { try { await connection.query("SELECT RELEASE_LOCK(?)", [LOCK_NAME]); } catch {} }
    await connection.end();
  }
}

if (require.main === module) finalize().catch((error) => { console.error("Opening Deposit reservation trigger finalization failed."); console.error(error.message); process.exit(1); });
