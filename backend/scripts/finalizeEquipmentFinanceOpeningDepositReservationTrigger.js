...
async function finalize() {
  ...
  try {
    ...
    if (!databaseName || !expected || databaseName !== expected) {
      throw new Error(`Connected database ${databaseName || "(unknown)"} does not match CHALIN03_EXPECTED_DATABASE.`);
    }
    ...
    if (!/v_deposit_received\s*\+\s*0\.01\s*<\s*v_deposit_required/i.test(action)) {
      throw new Error("The reset-safe reservation trigger was not finalized to the approved validator-compatible definition.");
    }
