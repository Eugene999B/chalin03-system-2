...
function has(sql, pattern, label) {
  if (!pattern.test(sql)) throw new Error(`Finance production contract failed: ${label}.`);
}
...
async function verify() {
  ...
  try {
    ...
    if (!databaseName || databaseName !== expected) {
      throw new Error(
        `Finance production contract verification refused: connected database ${databaseName || "(unknown)"} does not match expected production database ${expected || "(unset)"}.`
      );
    }
    ...
    if (missingColumns.length) {
      throw new Error(`Finance production contract is missing columns: ${missingColumns.join(", ")}.`);
    }
    ...
    for (const [triggerName, expectedEvent] of Object.entries(REQUIRED_TRIGGERS)) {
      ...
      if (row.EVENT_MANIPULATION !== expectedEvent || row.ACTION_TIMING !== "BEFORE") {
        throw new Error(`Finance production contract has invalid timing/event for ${triggerName}.`);
      }
    }
    ...
    has(paymentSql, /NEW\.payment_stage\s*=\s*['\"]opening_deposit['\"]/i, "opening-deposit payment stage gate");
    has(paymentSql, /NEW\.idempotency_key/i, "opening-deposit idempotency enforcement");
    has(paymentSql, /application_status/i, "explicit Finance application approval evidence");

    has(reservationSql, /NEW\.lock_status\s*=\s*['\"]installment_active['\"]/i, "exact installment reservation status gate");
    has(reservationSql, /v_deposit_received\s*\+\s*0\.01\s*<\s*v_deposit_required/i, "completed-deposit reservation gate");
    has(reservationSql, /hire_contract_assets/i, "Hire conflict protection");

    has(commitmentSql, /OLD\.equipment_commitment_status\s*<>\s*['\"]reserved['\"]/i, "commitment transition gate");
    has(commitmentSql, /NEW\.agreement_status\s+IN\s*\(/i, "active Finance agreement status gate");
    has(commitmentSql, /OLD\.agreement_status\s+NOT\s+IN\s*\(/i, "agreement status transition guard");
    has(commitmentSql, /v_active_lock_count/i, "active reservation evidence");
    has(commitmentSql, /v_active_hire_count/i, "Hire conflict protection");
    ...
    if (Number(migrationTable?.present || 0) !== 1) {
      throw new Error("Finance production contract is missing schema_migrations.");
    }
    ...
    if (missingMigrations.length) {
      throw new Error(`Finance production contract is missing migrations: ${missingMigrations.join(", ")}.`);
    }
      ...
      JSON.stringify(
        {
          ...
          required_columns: Object.entries(REQUIRED_COLUMNS).flatMap(([table, columns]) =>
            columns.map((column) => `${table}.${column}`)
          ),
          verified_triggers: triggerNames,
          verified_migrations: REQUIRED_MIGRATIONS,
          message: "Finance production contract verified after canonical pre-deploy repair and trigger finalization.",
        },
        null,
        2
      )
...
verify().catch((error) => {
  console.error("Finance production contract verification failed.");
