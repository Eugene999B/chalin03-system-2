...
const RELEASE_CONFIRMATION = "20260807_CHALIN_ONE_DOCUMENT_INTELLIGENCE";
const MIGRATION_RECORD = "20260807_chalin_one_document_intelligence";
const MIGRATION_LOCK_NAME = "chalin03:chalin-one:document-intelligence:20260807";
const MIGRATION_FILE = "20260807_chalin_one_document_intelligence.sql";
const VERIFIER_FILE = "20260807_chalin_one_document_intelligence_verify.sql";
...
function assertExecutionGates(env = process.env) {
  if (!truthy(env.CHALIN_ONE_ALLOW_DOCUMENT_INTELLIGENCE_SCHEMA_MIGRATION)) {
    throw new Error(
      "Set CHALIN_ONE_ALLOW_DOCUMENT_INTELLIGENCE_SCHEMA_MIGRATION=true only for a controlled CHALIN ONE document-intelligence migration rehearsal or approved release operation."
    );
  }
  if (
    String(env.CHALIN_ONE_DOCUMENT_INTELLIGENCE_MIGRATION_CONFIRM || "").trim() !==
    RELEASE_CONFIRMATION
  ) {
    throw new Error(
      `Set CHALIN_ONE_DOCUMENT_INTELLIGENCE_MIGRATION_CONFIRM=${RELEASE_CONFIRMATION} for this exact migration.`
    );
  }
  ...
  if (!databaseName) {
    throw new Error("Document-intelligence migration requires an explicit database name.");
  }
  if (environment === "production") {
    if (!truthy(env.CHALIN03_SIGNED_BACKUP_CONFIRMED)) {
      throw new Error(
        "Production document-intelligence migration requires a verified signed Professional Backup."
      );
    }
    if (!truthy(env.CHALIN03_SQL_BACKUP_CONFIRMED)) {
      throw new Error(
        "Production document-intelligence migration requires a separately verified SQL backup."
      );
    }
  } else if (!SAFE_NON_PRODUCTION_DATABASE.test(databaseName)) {
    throw new Error(
      "Non-production document-intelligence migration may target only isolated CHALIN ONE acceptance, staging or development databases."
    );
  }
...
function readMigrationFile(filename) {
  ...
  if (!fs.existsSync(filePath)) {
    throw new Error(`Approved document-intelligence migration file is missing: ${filePath}`);
  }
...
async function verifyTables(connection) {
  ...
  if (missing.length > 0) {
    throw new Error(`Document-intelligence migration is incomplete: ${missing.join(", ")}.`);
  }
  for (const tableName of EXPECTED_TABLES) {
    ...
    if (!String(row.table_collation || "").startsWith("utf8mb4_")) {
      throw new Error(`${tableName} must use utf8mb4 collation.`);
    }
  }
  return rows;
}

async function verifySafety(connection) {
  ...
  if (forbidden.length > 0) {
    throw new Error(
      `Document-intelligence tables contain forbidden secret columns: ${forbidden
        .map((row) => `${row.table_name}.${row.column_name}`)
        .join(", ")}.`
    );
  }

  const [[rawBinary]] = await connection.query(
    "SELECT COUNT(*) AS total FROM ai_knowledge_documents WHERE raw_binary_stored <> 0"
  );
  if (Number(rawBinary?.total || 0) !== 0) {
    throw new Error("Document-intelligence safety check found raw binary storage rows.");
  }

  return Object.freeze({ secret_columns_present: false, raw_binary_rows: 0 });
}

async function runChalinOneDocumentIntelligenceMigration({ env = process.env } = {}) {
  ...
  try {
    ...
    if (Number(lock?.acquired || 0) !== 1) {
      throw new Error("Could not acquire the CHALIN ONE document-intelligence migration lock.");
    }

    await executeSqlScript(
      connection,
      readMigrationFile(MIGRATION_FILE),
      "CHALIN ONE document-intelligence migration"
    );
    const tables = await verifyTables(connection);
    const safety = await verifySafety(connection);
    await executeSqlScript(
      connection,
      readMigrationFile(VERIFIER_FILE),
      "CHALIN ONE document-intelligence verifier"
    );
    ...
    if (!records[0]) {
      throw new Error("Document-intelligence migration record is missing.");
    }
...
if (require.main === module) {
  runChalinOneDocumentIntelligenceMigration()
    .then((result) => {
      console.log(
        `CHALIN ONE document intelligence verified ${result.verified_table_count}/${result.expected_table_count} tables on ${result.database}.`
      );
    })
    .catch((error) => {
      console.error(`CHALIN ONE document-intelligence migration failed: ${error.message}`);
      process.exitCode = 1;
    });
}
