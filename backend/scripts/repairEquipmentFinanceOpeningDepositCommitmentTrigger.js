...
async function main() {
  ...
  try {
    ...
    if (!db?.database_name || !expected || db.database_name !== expected) {
      throw new Error(`Opening Deposit trigger repair database identity check failed: ${db?.database_name || "unknown"}.`);
    }
    ...
    if (!trigger || trigger.EVENT_MANIPULATION !== "UPDATE" || trigger.ACTION_TIMING !== "BEFORE") {
      throw new Error("Opening Deposit commitment trigger verification failed.");
    }

    console.log(`Opening Deposit commitment trigger corrected on ${db.database_name}.`);
  } finally {
