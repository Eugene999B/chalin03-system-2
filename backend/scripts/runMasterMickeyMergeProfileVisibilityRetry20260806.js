...
const REPAIR_LOCK = "chalin03:master-mickey-merge-profile-visibility-retry:20260806";
...
function assertSnapshotChange(before, after, createdProfile) {
  const expectedCustomerDelta = createdProfile ? 1 : 0;
  const expectedUnlinkedDelta = createdProfile ? 1 : 0;
  if (after.customer_count !== before.customer_count + expectedCustomerDelta) {
    throw new Error("Unexpected customer profile count change.");
  }
  if (after.unlinked_sale_count !== before.unlinked_sale_count - expectedUnlinkedDelta) {
    throw new Error("Unexpected unlinked sale count change.");
  }
  if (after.unlinked_debt_count !== before.unlinked_debt_count - expectedUnlinkedDelta) {
    throw new Error("Unexpected unlinked debt count change.");
  }
  for (const field of [
    "sale_count",
    "debt_count",
...
async function validateTarget(connection, target) {
  if (normalizeName(target.sale_customer_name) !== TARGET_NAME || normalizeName(target.debt_customer_name) !== TARGET_NAME) {
    throw new Error("The target receipt identity is not exactly MASTER MICKEY.");
  }
  ...
  if ((target.sale_customer_id == null) !== (target.debt_customer_id == null)) {
    throw new Error("The target sale and debt customer links disagree; no guess was made.");
  }
...
async function verifyLinkedProfile(connection, target, customerId) {
  const [[row]] = await connection.query(
    `SELECT
       c.id AS customer_id,
       c.name,
  ...
  if (!row || normalizeName(row.name) !== TARGET_NAME || cleanText(row.phone, 100)) {
    throw new Error("The receipt-owned no-phone Master Mickey profile is invalid.");
  }
  if (Number(row.sale_customer_id) !== customerId || Number(row.debt_customer_id) !== customerId) {
    throw new Error("The exact July 31 receipt was not linked to one profile.");
  }
  if (
    ...
    throw new Error("The exact receipt changed financially while becoming merge-visible.");
  }
}

async function runMasterMickeyMergeProfileVisibilityRetry20260806() {
  ...
  try {
    ...
    if (!lockAcquired) throw new Error("Could not acquire the Mickey visibility retry lock.");
    ...
    if (target.sale_customer_id != null && target.debt_customer_id != null) {
      if (Number(target.sale_customer_id) !== Number(target.debt_customer_id)) {
        throw new Error("The target sale and debt are linked to different customers.");
      }
      customerId = Number(target.sale_customer_id);
    } else {
      ...
      if (!customerId) throw new Error("The merge-visible Master Mickey profile was not created.");
      createdProfile = true;
    ...
    await verifyLinkedProfile(connection, target, customerId);
    const after = await snapshot(connection);
    assertSnapshotChange(before, after, createdProfile);
    ...
    const summary = {
      ...
      existing_mickey_profile_count_required: false,
    ...
    await connection.query(
      ...
      [target.branch_id, `Linked ${TARGET_RECEIPT} to merge-visible customer #${customerId} without changing money or payment history.`]
    );
    await connection.query(
...
if (require.main === module) {
  runMasterMickeyMergeProfileVisibilityRetry20260806().catch((error) => {
    console.error("Master Mickey merge profile visibility retry failed safely. No partial change was saved.");
...
module.exports = {
  ...
  normalizeName,
  runMasterMickeyMergeProfileVisibilityRetry20260806,
  validateTarget,
};