...
async function run() {
  ...
  try {
    ...
    const [rows] = await connection.query(
      "SELECT id, agreement_terms FROM equipment_finance_settings WHERE id = 1 LIMIT 1 FOR UPDATE"
    );
    ...
    if (currentTerms !== LEGACY_TERMS) {
      ...
      console.log(JSON.stringify({
        ...
        reason: "The Finance Settings agreement terms are no longer the legacy default, so the repair did not overwrite an existing user edit.",
      }));
      return;
    }
    ...
    if (String(verify?.agreement_terms || "") !== NEW_TERMS) {
      throw new Error("Agreement terms verification failed after update.");
    }
    if (String(verify?.terms_version || "") !== "FIN-TERMS-20260831") {
      throw new Error("Agreement terms version verification failed after update.");
    }
