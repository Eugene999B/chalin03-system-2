...
canonicalRouter.get(
  ...
  async (_req, res) => {
    ...
    try {
      ...
      return res.status(Number(error?.statusCode || 500) >= 500 ? Number(error?.statusCode || 500) : 500).json({
        status: "error",
...
// Express keeps router layers in registration order. Put the canonical route
// ahead of the legacy compatibility router so the duplicate readiness gate
// cannot intercept this endpoint first.
