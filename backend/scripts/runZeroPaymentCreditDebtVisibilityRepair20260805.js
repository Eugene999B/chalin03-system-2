...
function assertProtectedDebtsUnchanged(before, after) {
  ...
  if (left !== right) {
    throw new Error("A paid, partially paid, or payment-linked debt changed. The repair was rolled back.");
  }
}
...
async function coreSnapshot(connection) {
  ...
  const dailyClosingSelect = hasDailyClosings
    ? `,
       (SELECT COUNT(*) FROM daily_closings) AS daily_closing_count,
       (SELECT COALESCE(SUM(total_sales), 0) FROM daily_closings) AS daily_closing_sales_total`
    : `,
       0 AS daily_closing_count,
       0 AS daily_closing_sales_total`;
  ...
  return {
    ...
    daily_closing_sales_total: money(row.daily_closing_sales_total),
  };
}
  ...
  const [rows] = await connection.query(
    `SELECT
     ...
     WHERE d.branch_id = ?
       AND d.sale_id = ?
     GROUP BY
       d.id, d.customer_id, d.customer_name, d.customer_phone,
       d.amount_owed, d.amount_paid, d.balance, d.status, d.due_date
     ORDER BY d.id
     FOR UPDATE`,
...
function isProtectedDebt(debt) {
  return (
    ...
    String(debt?.status || "").toLowerCase() === "partial" ||
  ...
  for (const saleId of repairedSaleIds) {
    const [[row]] = await connection.query(
      `SELECT
         ...
         COALESCE(SUM(CASE WHEN d.balance > 0.005 AND d.status = 'unpaid' THEN 1 ELSE 0 END), 0) AS visible_open_count,
         COALESCE(SUM(dp.payment_count), 0) AS payment_count
       FROM debts d
       LEFT JOIN (
    ...
    if (Number(row.visible_open_count || 0) !== 1) {
      throw new Error(`Repaired sale ${saleId} is still not visible as one unpaid open debt.`);
    }
...
async function runZeroPaymentCreditDebtVisibilityRepair20260805() {
  ...
  try {
    ...
    if (!lockAcquired) throw new Error("Could not acquire the zero-payment credit debt repair lock.");

    if (!(await migrationRecordExists(connection, REQUIRED_BACKFILL_RECORD))) {
      throw new Error("The missing-credit-debt backfill must complete before this visibility repair.");
    }
        ...
        `UPDATE debts
         ...
         WHERE id = ?
           ...
           AND NOT EXISTS (
             ...
             WHERE dp.branch_id = debts.branch_id
               AND dp.debt_id = debts.id
           )`,
        [
          sale.customer_id || null,
          customerName,
    ...
    await connection.query(
      `INSERT INTO activity_log (
         ...
         NULL, NULL, 'RESTORE_ZERO_PAYMENT_CREDIT_DEBT_VISIBILITY_20260805', ?,
         'spare_parts', 'credit_debt_visibility_repair', ?,
...
if (require.main === module) {
  runZeroPaymentCreditDebtVisibilityRepair20260805().catch((error) => {
    console.error("Zero-payment credit debt visibility repair failed safely. No partial repair was saved.");
