...
const RELEASE_CONFIRMATION = "20260809_CHALIN_ONE_PUBLIC_REDIRECTS";
const MIGRATION_RECORD = "20260809_chalin_one_public_redirects";
const MIGRATION_LOCK_NAME = "chalin03:chalin-one:public-redirects:20260809";
const MIGRATION_FILE = "20260809_chalin_one_public_redirects.sql";
const VERIFIER_FILE = "20260809_chalin_one_public_redirects_verify.sql";
const EXPECTED_TABLE = "public_redirect_rules";
...
function assertVerifiedStagingIsolation(env) {
  return validateFullStagingEnvironment(
    {
      ...env,
      CHALIN_ONE_ALLOW_PUBLIC_REDIRECT_MIGRATION: "",
    },
    { mode: "runtime" }
  );
}
...
function assertExecutionGates(env = process.env) {
  if (!truthy(env.CHALIN_ONE_ALLOW_PUBLIC_REDIRECT_MIGRATION)) {
    throw new Error(
      "Set CHALIN_ONE_ALLOW_PUBLIC_REDIRECT_MIGRATION=true only for an isolated CHALIN ONE staging/acceptance rehearsal or separately approved release operation."
    );
  }
  if (
    String(env.CHALIN_ONE_PUBLIC_REDIRECT_MIGRATION_CONFIRM || "").trim() !==
    RELEASE_CONFIRMATION
  ) {
    throw new Error(
      `Set CHALIN_ONE_PUBLIC_REDIRECT_MIGRATION_CONFIRM=${RELEASE_CONFIRMATION} for this exact migration.`
    );
  }
  ...
  if (!databaseName) {
    throw new Error("Public redirect migration requires an explicit database name.");
  }
  ...
  if (environment === "production") {
    if (
      ...
      throw new Error(
        "Production public redirect migration requires both verified Professional and SQL backups."
      );
    }
  } else if (!SAFE_NON_PRODUCTION_DATABASE.test(databaseName)) {
    if (environment !== "staging") {
      throw new Error(
        "Non-production public redirect migration may target only isolated CHALIN ONE acceptance, staging or development databases."
      );
    }
    try {
      ...
      throw new Error(
        `Public redirect migration refused the nonstandard staging database because dedicated CHALIN ONE isolation could not be verified: ${error.message}`
      );
    }
  }
...
function readMigrationFile(filename) {
  ...
  if (!fs.existsSync(filePath)) {
    throw new Error(`Public redirect migration source is missing: ${filePath}`);
  }
...
async function verifyFoundation(connection) {
  ...
  if (!tables[0]) {
    throw new Error("Public redirect migration is incomplete: public_redirect_rules is missing.");
  }
  ...
  if (Number(invalid?.total || 0) !== 0) {
    throw new Error("Public redirect foundation contains invalid lifecycle or status-code rows.");
  }

  return { table_count: 1, invalid_rows: 0 };
}

async function runChalinOnePublicRedirectMigration({ env = process.env } = {}) {
  ...
  try {
    ...
    if (Number(lock?.acquired || 0) !== 1) {
      throw new Error("Could not acquire the CHALIN ONE public redirect migration lock.");
    }

    await executeSqlScript(
      connection,
      readMigrationFile(MIGRATION_FILE),
      "CHALIN ONE public redirect migration"
    );
    const verified = await verifyFoundation(connection);
    await executeSqlScript(
      connection,
      readMigrationFile(VERIFIER_FILE),
      "CHALIN ONE public redirect verifier"
    );
    ...
    if (!records[0]) {
      throw new Error("Public redirect migration record is missing.");
    }
...
if (require.main === module) {
  runChalinOnePublicRedirectMigration()
    .then((result) =>
      console.log(
        `CHALIN ONE public redirect foundation verified on ${result.database}.`
      )
    )
    .catch((error) => {
      console.error(`CHALIN ONE public redirect migration failed: ${error.message}`);
      process.exitCode = 1;
    });
}
