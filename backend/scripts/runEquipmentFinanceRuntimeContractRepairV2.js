...
async function main() {
  ...
  try {
    ...
    for (const tableName of [
      ...
      if (rows[0] && String(rows[0].IS_NULLABLE).toUpperCase() !== "YES") {
        await connection.query(`ALTER TABLE ${tableName} MODIFY COLUMN hire_location_id INT NULL`);
      }
    }

    // The approved first due date is stored on the quotation, not on the credit application.
    // The agreement points to that quotation through quotation_id.
    await connection.query(
      `UPDATE equipment_sale_agreements agreement
       LEFT JOIN equipment_sales_quotations quotation
         ON quotation.id = agreement.quotation_id
       SET agreement.first_due_date = COALESCE(
             CASE
               WHEN quotation.proposed_first_due_date IS NOT NULL
                AND quotation.proposed_first_due_date >= DATE(agreement.created_at)
               THEN quotation.proposed_first_due_date
             END,
    ...
    await connection.query(
      ...
      [
        MIGRATION_NAME,
        "Runtime Finance contract normalization using quotation first-due-date evidence, payment ENUMs, nullable Finance origin fields, impossible-date repair and recurrence guards.",
      ]
    );
