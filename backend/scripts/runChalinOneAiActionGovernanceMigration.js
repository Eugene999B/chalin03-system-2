...
const RELEASE_CONFIRMATION = "20260806_CHALIN_ONE_AI_ACTION_GOVERNANCE";
const MIGRATION_RECORD = "20260806_chalin_one_ai_action_governance";
const MIGRATION_LOCK_NAME = "chalin03:chalin-one:ai-action-governance:20260806";
const MIGRATION_FILE = "20260806_chalin_one_ai_action_governance.sql";
const VERIFIER_FILE = "20260806_chalin_one_ai_action_governance_verify.sql";
...
function connectionOptions(env = process.env) {
  return {
    ...
    ssl:
      String(env.DB_SSL || "").toLowerCase() === "true"
        ? {
...
function assertExecutionGates(env = process.env) {
  if (!truthy(env.CHALIN_ONE_ALLOW_AI_ACTION_SCHEMA_MIGRATION)) {
    throw new Error(
      "Set CHALIN_ONE_ALLOW_AI_ACTION_SCHEMA_MIGRATION=true only for a controlled action-governance migration rehearsal or approved release operation."
    );
  }
  if (
    String(env.CHALIN_ONE_AI_ACTION_MIGRATION_CONFIRM || "").trim() !==
    RELEASE_CONFIRMATION
  ) {
    throw new Error(
      `Set CHALIN_ONE_AI_ACTION_MIGRATION_CONFIRM=${RELEASE_CONFIRMATION} for this exact migration.`
    );
  }
  ...
  if (!databaseName) {
    throw new Error("AI action-governance migration requires an explicit database name.");
  }
  if (environment === "production") {
    if (!truthy(env.CHALIN03_SIGNED_BACKUP_CONFIRMED)) {
      throw new Error(
        "Production action-governance migration requires a verified signed Professional Backup."
      );
    }
    if (!truthy(env.CHALIN03_SQL_BACKUP_CONFIRMED)) {
      throw new Error(
        "Production action-governance migration requires a separately verified SQL backup."
      );
    }
  } else if (!SAFE_NON_PRODUCTION_DATABASE.test(databaseName)) {
    throw new Error(
      "Non-production AI action migration may target only isolated CHALIN ONE acceptance, staging or development databases."
    );
  }
...
function readMigrationFile(filename) {
  ...
  if (!fs.existsSync(filePath)) {
    throw new Error(`Approved AI action migration file is missing: ${filePath}`);
  }
...
async function verifyTables(connection) {
  ...
  if (missing.length > 0) {
    throw new Error(`AI action governance migration is incomplete: ${missing.join(", ")}.`);
  }
...
async function verifyNoSecretColumns(connection) {
  ...
  if (forbidden.length > 0) {
    throw new Error(
      `AI action governance tables contain forbidden secret columns: ${forbidden
...
async function runChalinOneAiActionGovernanceMigration({
  env = process.env,
} = {}) {
  ...
  try {
    ...
    if (Number(lock?.acquired || 0) !== 1) {
      throw new Error("Could not acquire the CHALIN ONE AI action migration lock.");
    }
    await executeSqlScript(
      connection,
      readMigrationFile(MIGRATION_FILE),
      "CHALIN ONE AI action governance migration"
    );
    ...
    await executeSqlScript(
      ...
      "CHALIN ONE AI action governance verifier"
    );
    ...
    if (!records[0]) {
      throw new Error("AI action governance migration record is missing.");
    }
    const [[executed]] = await connection.query(
      `SELECT COUNT(*) AS row_count FROM ai_action_proposals
       WHERE proposal_status = 'executed'`
    );
    if (Number(executed?.row_count || 0) !== 0) {
      throw new Error(
        "Action-governance foundation acceptance requires zero executed proposals."
      );
    }
    return Object.freeze({
...
if (require.main === module) {
  runChalinOneAiActionGovernanceMigration()
    .then((result) => {
      console.log(
        `CHALIN ONE AI action governance verified ${result.verified_table_count}/${result.expected_table_count} tables with zero executed proposals on ${result.database}.`
      );
    })
    .catch((error) => {
      console.error(`CHALIN ONE AI action migration failed: ${error.message}`);
      process.exitCode = 1;
    });
}
