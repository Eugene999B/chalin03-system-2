...
async function runOperationalSchemaAcceptance() {
  ...
  try {
    ...
    for (const migration of STAGING_OPERATIONAL_MIGRATIONS) {
      migrationReports.push(await applyStagingMigration(connection, migration));
    }
    ...
    if (Number(migrationCount?.migration_count || 0) !== STAGING_OPERATIONAL_MIGRATIONS.length) {
      throw new Error("Not every Payroll/Inventory staging migration was durably recorded.");
    }
      ...
      JSON.stringify(
        {
          ...
          operational_migrations: migrationReports,
