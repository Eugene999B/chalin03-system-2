...
const MIGRATION_LOCK = "chalin03:equipment-finance:phase5";
const MIGRATION_RECORD = "equipment_finance_phase5_documents_delivery";
const MIGRATION_FILE = "20260801_equipment_finance_phase5_documents_delivery.sql";
const VERIFIER_FILE = "20260801_equipment_finance_phase5_documents_delivery_verify.sql";
...
function readMigrationFile(filename) {
  ...
  if (!fs.existsSync(filePath)) throw new Error(`Approved Phase 5 SQL file is missing: ${filePath}`);
...
function validatePhaseFiveSchema(results) {
  if (results.length !== 5) {
    throw new Error(`Phase 5 verifier returned ${results.length} result sets instead of 5.`);
  }
  const [migrationRows, tableRows, policyRows, columnRows, invalidRows] = results;
  if (migrationRows.length !== 1 || migrationRows[0].migration_name !== MIGRATION_RECORD) {
    throw new Error("The Phase 5 migration record was not verified.");
  }
  const tables = new Set(tableRows.map((row) => String(row.TABLE_NAME || "")));
  const missing = REQUIRED_TABLES.filter((table) => !tables.has(table));
  if (missing.length) throw new Error(`Phase 5 tables are missing: ${missing.join(", ")}.`);
  if (Number(policyRows[0]?.policy_rows || 0) !== 1) {
    throw new Error("The Phase 5 document and delivery policy singleton was not verified.");
  }
  if (columnRows.length !== 32) {
    throw new Error(`The Phase 5 protected column contract returned ${columnRows.length}; expected 32.`);
  }
  if (Number(invalidRows[0]?.invalid_control_rows || 0) !== 0) {
    throw new Error("The Phase 5 verifier found invalid review, authorization or confirmation rows.");
  }
}

async function runEquipmentFinancePhaseFiveStartup() {
  ...
  try {
    ...
    if (!lockAcquired) throw new Error("Could not acquire the Phase 5 migration lock.");

    if (!(await migrationRecordExists(connection))) {
      await executeStatements(
        connection,
        splitSqlScript(readMigrationFile(MIGRATION_FILE)),
        "Equipment Finance Phase 5 migration"
      );
    }
    const results = await executeStatements(
      connection,
      splitSqlScript(readMigrationFile(VERIFIER_FILE)),
      "Equipment Finance Phase 5 verifier"
    );
    validatePhaseFiveSchema(results);
...
if (require.main === module) {
  runEquipmentFinancePhaseFiveStartup().catch((error) => {
    console.error("Equipment Finance Phase 5 Railway startup gate failed.");
...
module.exports = {
  ...
  runEquipmentFinancePhaseFiveStartup,
  splitSqlScript,
  validatePhaseFiveSchema,
