  ...
  async (req, res) => {
    ...
    try {
      ...
      const [products] = await connection.query(
        `SELECT
           id, branch_id, name, quantity, cost_price,
      ...
      const shouldUpdateCost =
        update_cost_price === true || update_cost_price === "true";
      const newCostPrice =
...
// All other product routes retain the established implementation. The two
// quantity-changing routes above are intentionally registered first so an
// enforced serialized product can never fall through to the legacy handlers.
