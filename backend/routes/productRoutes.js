...
router.post("/", requireAuth, requireRole("admin", "manager"), async (req, res) => {
  try {
    ...
    if (lowStockThreshold === null) {
      return res.status(400).json({
        status: "error",
        message: "Low-stock threshold must be a whole number and cannot be negative.",
      });
    }
    ...
    await logActivity(
      req.user.id,
      "CREATE_PRODUCT",
      `Created product "${name}" with quantity ${productQuantity}`
    );
...
router.put("/:id", requireAuth, requireRole("admin", "manager"), async (req, res) => {
  try {
    ...
    await logActivity(
      req.user.id,
      "UPDATE_PRODUCT",
      `Updated product "${name}" with ID ${id}`
    );
    ...
    try {
      ...
      await connection.query(
        ...
        [
          req.user.id,
          "STOCK_ADJUSTMENT",
          `Product "${product.name}" stock changed from ${product.quantity} to ${newQuantity}. Reason: ${reason}`,
        ]
      );
...
// Admin only. This is a soft delete.
