...
async function writeSmsLogSafe({
  ...
  try {
    ...
    console.warn("Sale security SMS log skipped:", error.message);
  }
}
...
async function sendSaleVoidedSecuritySmsAlert({
  ...
  try {
    ...
    if (!alertPhone) {
      console.warn(
        "Sale voided SMS alert skipped: no valid owner/admin phone found."
      );
      return;
    }
    ...
    const alertMessage = truncateMessage(
      `${businessName}: Security alert. Sale ${
    ...
    try {
      ...
      console.warn("Sale voided SMS alert failed:", error.message);
    }
  } catch (error) {
