...
function mutationBlockedResponse(res, conflict, action) {
  ...
  const productText = conflict?.product_name ? ` ${conflict.product_name}` : " This sale";
...
router.put("/:id", requireRole("admin"), async (req, res, next) => {
  try {
    ...
    return res.status(500).json({
      ...
      message: "The sale could not be checked safely for serialized physical identities. No edit was allowed.",
    });
  }
});
...
router.patch("/:id/void", requireRole("admin"), async (req, res, next) => {
  try {
    ...
    return res.status(500).json({
      ...
      message: "The sale could not be checked safely for serialized physical identities. No void was allowed.",
    });
  }
});
