...
router.patch("/customer/:customerId/identity", requireRole("admin", "manager"), async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const [customers] = await connection.query(`SELECT id, name, phone, location FROM customers WHERE id = ? AND branch_id = ? LIMIT 1 FOR UPDATE`, [customerId, branchId]);
      ...
      const [debtRows] = await connection.query(`SELECT COUNT(*) AS debt_count FROM debts WHERE branch_id = ? AND customer_id = ?`, [branchId, customerId]);
      ...
      await connection.query(`UPDATE customers SET name = ?, phone = ?, location = ? WHERE id = ? AND branch_id = ?`, [name, phone, location, customerId, branchId]);
      await writeAuditEvent({
        connection,
        req,
        branchId,
