...
function validCustomerName(value) {
  ...
  return parts.length >= 2 ? name : null;
}

function customerNameIsValid(value) {
  const name = cleanText(value, 150);
...
// GET /api/debt-customers/feature-controls
router.get("/feature-controls", async (req, res) => {
  try {
    ...
    return res.status(500).json({
      status: "error",
      message: "Could not load customer data feature controls.",
    });
  }
});

// PUT /api/debt-customers/feature-controls
router.put(
  "/feature-controls",
  requireRole("admin"),
  async (req, res) => {
    try {
      ...
      return res.json({
        status: "success",
        message: "Customer data feature controls updated successfully.",
        controls,
      });
    } catch (error) {
      ...
      return res.status(500).json({
        status: "error",
        message: "Could not update customer data feature controls.",
      });
    }
  }
);

// PATCH /api/debt-customers/customer/:customerId/identity
router.patch(
  ...
  async (req, res) => {
    ...
    if (!customerId) {
      return res.status(400).json({
        ...
        message: "Customer ID must be a positive whole number.",
      });
    }
    ...
    try {
      ...
      if (!controls.customer_identity_editing_enabled) {
        return res.status(403).json({
          status: "error",
          code: "CUSTOMER_IDENTITY_EDITING_DISABLED",
          message: getFeatureDisabledMessage("customer_identity_editing"),
        });
      }
      ...
      if (!name) {
        return res.status(400).json({
          status: "error",
          code: "CUSTOMER_NAME_REQUIRES_TWO_NAMES",
          message:
            "Customer name must contain at least two separate names, for example Appiah Eugene.",
        });
      }
      ...
      try {
        ...
        if (debtCount > 0 && !phone) {
          ...
          return res.status(400).json({
            status: "error",
            code: "DEBT_CUSTOMER_PHONE_REQUIRED",
            message:
              "This customer has debt records, so a phone number is required before the identity can be saved.",
          });
        }
      ...
      return res.status(500).json({
        status: "error",
        message: "Could not update the selected customer details.",
      });
    }
  }
);
